...
export class ProfileAnalysisService {
   ...
   */
  public async getProfileAnalysis(cuttingListId?: string): Promise<ProfileAnalysisResult> {
    try {
      ...
      if (cuttingListId) {
        ...
        if (cuttingListData.success && cuttingListData.data) {
          ...
          (sections as CuttingListSection[]).forEach((section: CuttingListSection) => {
            if (section.items && Array.isArray(section.items)) {
              section.items.forEach((item: CuttingListItem) => {
                ...
                profileTypeCounts[profileType] = (profileTypeCounts[profileType] || 0) + 1;
          ...
          const profileTypesTemp = Object.entries(profileTypeCounts).map(([type, count]) => ({
            name: type,
            count,
            percentage: ((count / totalItems) * 100).toFixed(1)
          }));
          
          const sizeDistributionTemp = Object.entries(sizeCounts).map(([size, count]) => ({
            size,
            count,
            percentage: ((count / totalItems) * 100).toFixed(1)
          }));
          ...
          profileStats = {
            ...
            averageQuantityPerProfile: totalItems > 0 ? parseFloat((totalQuantity / totalItems).toFixed(1)) : 0,
            mostUsedProfileType: Object.entries(profileTypeCounts).sort(([,a], [,b]) => b - a)[0]?.[0] || 'Bilinmiyor',
            mostUsedSize: Object.entries(sizeCounts).sort(([,a], [,b]) => b - a)[0]?.[0] || 'Bilinmiyor'
          };
        }
      } else {
      ...
      logger.info('Fetched profile analysis statistics', { stats: profileStats });
      return profileStats;
    } catch (error) {

...
export class WorkOrderAnalysisService {
  ...
  private getWeekStartDate(weekNumber: number, year: number): Date {
    ...
    logger.info(`Business Week ${weekNumber} of ${year} calculated`, {
      ...
      weekEndDate: new Date(targetWeekStart.getTime() + 5 * 24 * 60 * 60 * 1000)
        .toISOString()
        .split("T")[0], // 6 gün (Pazartesi-Cumartesi)
    });
   ...
   */
  public async getWorkOrderAnalysis(
    cuttingListId?: string,
  ): Promise<WorkOrderAnalysisResult> {
    try {
      ...
      if (cuttingListId) {
        ...
        if (cuttingListData.success && cuttingListData.data) {
          ...
          const statusDistributionTemp = Object.entries(statusCounts).map(
            ([status, count]) => ({
              status: status,
              count,
          ...
          const priorityDistributionTemp = Object.entries(priorityCounts).map(
            ([priority, count]) => ({
              priority: priority,
              count,
              percentage: ((count / totalWorkOrders) * 100).toFixed(1),
          ...
          logger.info("Processing cutting list analysis:", {
            ...
            totalItems: (sections as CuttingListSection[]).reduce(
              (sum: number, section: CuttingListSection) =>
      ...
      logger.info("Fetched work order analysis statistics", {

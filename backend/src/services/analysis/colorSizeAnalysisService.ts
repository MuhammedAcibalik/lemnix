...
export class ColorSizeAnalysisService {
   ...
   */
  public async getColorSizeAnalysis(
    cuttingListId?: string,
  ): Promise<ColorSizeAnalysisResult> {
    try {
      ...
      if (cuttingListId) {
        ...
        if (cuttingListData.success && cuttingListData.data) {
          ...
          const colorAnalysisTemp = Object.entries(colorCounts)
            .map(([color, count]) => ({
              color,
              count,
          ...
          const sizeAnalysisTemp = Object.entries(sizeCounts)
            .map(([size, count]) => ({
              size,
              count,
              percentage: ((count / totalItems) * 100).toFixed(1),
          ...
          const combinationsTemp = Object.entries(combinationCounts)

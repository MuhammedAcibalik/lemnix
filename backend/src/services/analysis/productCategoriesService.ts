...
export class ProductCategoriesService {
   ...
   */
  public async getProductCategoriesAnalysis(
    cuttingListId?: string,
  ): Promise<ProductCategoriesAnalysis> {
    try {
      const categoryStats: ProductCategoriesAnalysis = {
        totalCategories: 0,
        categories: [],
      ...
      if (cuttingListId) {
        ...
        if (cuttingListData.success && cuttingListData.data) {
          ...
          const categoryStats_temp: Record<
            string,
            { items: number; totalQuantity: number }
          > = {};
          ...
          (sections as CuttingListSection[]).forEach(
            (section: CuttingListSection) => {
              ...
              const totalQuantity = section.items
                ? section.items.reduce(
                    (sum: number, item: CuttingListItem) =>
                      sum + (item.orderQuantity || 0),
                    0,
                  )
                : 0;

              if (!categoryStats_temp[categoryName]) {
                categoryStats_temp[categoryName] = {
                  items: 0,
                  totalQuantity: 0,
                };
              }
          ...
          const totalItems = Object.values(categoryStats_temp).reduce(
            (sum, cat) => sum + cat.items,
            0,
          );

          categoryStats.categories = Object.entries(categoryStats_temp).map(
            ([name, stats]) => ({
              name,
          ...
          categoryStats.totalCategories =
            Object.keys(categoryStats_temp).length;
          categoryStats.averageItemsPerCategory =
            categoryStats.totalCategories > 0
              ? parseFloat(
                  (totalItems / categoryStats.totalCategories).toFixed(1),
                )
              : 0;
          categoryStats.mostActiveCategory =
            Object.entries(categoryStats_temp).sort(
              ([, a], [, b]) => b.items - a.items,
            )[0]?.[0] || "Bilinmiyor";

          categoryStats.categoryPerformance = categoryStats.categories
            .map((category) => ({
            ...
            .sort((a, b) => {
              const aCategory = categoryStats.categories.find(
                (c) => c.name === a.category,
              );
              const bCategory = categoryStats.categories.find(
                (c) => c.name === b.category,
              );
        ...
        if (allListsData.success && allListsData.data) {
          ...
          const categoryStats_temp: Record<
            string,
            { items: number; totalQuantity: number; lists: number }
          > = {};
            ...
            (list: CuttingListData) => {
              ...
              (sections as CuttingListSection[]).forEach(
                (section: CuttingListSection) => {
                  ...
                  categoryStats_temp[categoryName].items += itemCount;
                  categoryStats_temp[categoryName].totalQuantity +=
                    totalQuantity;

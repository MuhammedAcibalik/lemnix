...
import { OptimizationItem, OptimizationResult, OptimizationAlgorithm, WasteCategory } from '../types';
...
export class ProfileOptimizationService {
   ...
   */
  public optimizeByProfileType(items: OptimizationItem[]): ProfileOptimizationResult[] {
    ...
    try {
      ...
      const poolingResult = this.advancedOptimizationService.optimizeWithPooling(items, 'pooling');
      
      // Convert to ProfileOptimizationResult format
      const result: ProfileOptimizationResult = {
        profileType: 'Pooled Optimization',
        stockLength: 6100,
        cuts: poolingResult.cuts.map((cut: any, index: number) => ({
          ...
          workOrders: cut.workOrderBreakdown?.map((b: any) => String(b.workOrderId)) || []
        })),
        totalWaste: poolingResult.totalWaste,
        efficiency: poolingResult.efficiency,
        totalCost: poolingResult.totalCost,
        workOrders: Array.from(new Set(poolingResult.cuts.flatMap((cut: any) => 
          cut.workOrderBreakdown?.map((b: any) => String(b.workOrderId)) || []
        )))
      };
      ...
      const profileGroups = this.extractProfileGroups(poolingResult);
   ...
   */
  private extractProfileGroups(result: any): ProfileGroup[] {
    const poolMap = new Map<string, ProfileGroup>();
    
    result.cuts.forEach((cut: any) => {
      ...
      if (cut.workOrderBreakdown) {
        cut.workOrderBreakdown.forEach((breakdown: any) => {
        ...
        if (!group.workOrders.includes(String(cut.workOrderId))) {
          group.workOrders.push(String(cut.workOrderId));
        }
      }
    });
    ...
    poolMap.forEach((group) => {
      const totalStockLength = result.cuts
        .filter((cut: any) => (cut.poolKey || 'default') === group.profileType)
        .reduce((sum: number, cut: any) => sum + (cut.stockLength || 0), 0);
  ...
  public mergeProfileResults(results: ProfileOptimizationResult[]): OptimizationResult {
    ...
    return {
      cuts: allCuts.map((cut, index) => ({
        ...
        materialType: results.find(r => r.cuts.includes(cut))?.profileType as any || 'Unknown',
        segments: cut.segments.map((s, segIndex) => ({
          id: `${s.workOrderId}-${s.length}`,
          cutId: cut.id,
          sequenceNumber: segIndex + 1,
          profileType: results.find(r => r.cuts.includes(cut))?.profileType as any || 'Unknown',
          length: s.length,
          quantity: s.quantity,

...
export class ExcelExportService {
  ...
  private generateExcelWorkbook(cuttingList: CuttingList): XLSX.WorkBook {
    ...
    const totalProfiles = cuttingList.sections.reduce(
      (total, section) =>
        total +
        section.items.reduce((sum, item) => sum + item.profiles.length, 0),
      0,
    );
    ...
    mainData.push([
      `Oluşturulma Tarihi: ${new Date(cuttingList.createdAt).toLocaleDateString("tr-TR")}`,
    ]);
    mainData.push([]); // Spacing
    ...
    cuttingList.sections.forEach((section, sectionIndex) => {
      ...
      // Section summary
      const sectionTotalProfiles = section.items.reduce(
        (sum, item) => sum + item.profiles.length,
        0,
      );
      mainData.push([]); // Spacing
      mainData.push([
        `Bölüm ${sectionIndex + 1} Toplamı: ${section.items.length} iş emri, ${sectionTotalProfiles} profil`,
      ]);
      mainData.push([]); // Spacing
      mainData.push([]); // Spacing
    });
    ...
    // ===== FOOTER SECTION =====
    mainData.push([
      `Bu belge ${new Date().toLocaleDateString("tr-TR")} tarihinde LEMNİX sisteminden oluşturulmuştur.`,
    ]);
    mainData.push(["LEMNİX - Alüminyum Profil Kesim Yönetim Sistemi"]);

    const mainWorksheet = XLSX.utils.aoa_to_sheet(
      mainData as (string | number | boolean | null)[][],
    );
    mainWorksheet["!cols"] = [
      { wch: 20 }, // İş Emri
      { wch: 15 }, // Tarih
    ...
    XLSX.utils.book_append_sheet(
      workbook,
      mainWorksheet,
      cuttingList.title.substring(0, 31),
    );
  ...
  private styleMainWorksheetPDF(
          ...
          for (let col = 0; col < 10; col++) {
            ...
            if (cell) {
              cell.s = {
                ...
                fill: {
                  fgColor: { rgb: profileIndex === 0 ? "FFFFFF" : "F8F9FA" },
                },
              };
            }
          }
    ...
    if (worksheet[`A${finalSummaryStart}`]) {
      ...
      if (!worksheet["!merges"]) worksheet["!merges"] = [];
      worksheet["!merges"].push({
        s: { r: finalSummaryStart - 1, c: 0 },
        e: { r: finalSummaryStart - 1, c: 9 },
      });
    }
    ...
    for (let col = 1; col < 4; col++) {
      ...
      const dataCell = XLSX.utils.encode_cell({
        r: finalSummaryStart + 1,
        c: col,
      });

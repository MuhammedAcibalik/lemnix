...
class SmartSuggestionService {
  ...
  private readonly databasePath = path.join(
    __dirname,
    "../../../smart_suggestion_database.json",
  );
  ...
  public getProductSuggestions(
    ...
    try {
      ...
      exactMatches.forEach(([product, data]) => {
        const confidence = this.calculateProductConfidence(
          product,
          data.frequency,
          db,
        );
        ...
        suggestions.push({
          ...
          context: {
            ...
            ...(data.commonProfiles && {
              relatedProfiles: data.commonProfiles,
            }),
      ...
      const confidenceLevel = this.calculateConfidenceLevel(suggestions);
    ...
    try {
      ...
      if (productName) {
        ...
        if (productData) {
          ...
          sortedSizes.forEach(([size, frequency]) => {
            const confidence = Math.min(
              100,
              (frequency / productData.totalOccurrences) * 100,
            );
        ...
        sizeMatches.forEach(([size, data]) => {
          const confidence = this.calculateSizeConfidence(
            size,
            data.frequency,
            db,
          );
          ...
          suggestions.push({
            ...
            context: {
              ...(data.commonProducts && {
                relatedProducts: data.commonProducts,
              }),
              ...(data.commonProfiles && {
                relatedProfiles: data.commonProfiles,
              }),
      ...
      if (productName && size) {
        ...
        if (comboData) {
          const sortedProfiles = Object.entries(comboData.profiles)
            .filter(
              ([profile]) => queryUpper === "" || profile.includes(queryUpper),
            )
            .sort((a, b) => b[1] - a[1])
            .slice(0, limit);
          ...
          sortedProfiles.forEach(([profile, frequency]) => {
            const confidence = Math.min(
              100,
              (frequency / comboData.totalUsage) * 100,
            );
            const avgQuantity =
              db.profileSuggestions[profile]?.averageQuantity || 1;
  ...
  public getAutoCompleteSuggestions(
    ...
    try {
      ...
      startMatches.forEach(([value, data]) => {
        const confidence = this.calculateAutoCompleteConfidence(
          type,
          value,
          data.frequency,
          db,
  ...
  private calculateAutoCompleteConfidence(
    ...
    const totalItems =
      type === "profile" ? db.metadata.totalProfiles : db.metadata.totalItems;
  ...
  /**
   * Calculate confidence level from average confidence score
   */

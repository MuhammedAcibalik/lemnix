...
export class EnterpriseProfileSuggestionService {
  ...
  public getSmartSuggestions(
    ...
    return {
      ...
      patternAnalysis: patternAnalysis as {
        mostCommonProfiles: string[];
        typicalMeasurements: string[];
        quantityPatterns: number[];
        seasonalTrends?: string[];
   ...
   */
  private analyzePatterns(patterns: ProfilePattern[]): Record<string, unknown> {
    if (patterns.length === 0) {
      return {
        mostCommonProfiles: [],
        typicalMeasurements: [],
  ...
  public getAvailableSizesForProduct(
    ...
    // If no sizes found, return default test sizes
    if (sizeMap.size === 0) {
      const defaultSizes = [
        { size: '24"X36"', frequency: 10, lastUsed: new Date().toISOString() },

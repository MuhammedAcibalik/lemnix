...
export class QuantityCalculationService {
    ...
    private convertEnterpriseToOldFormat(enterpriseData: Record<string, unknown>): ExcelAnalysisData {
        ...
        if (enterpriseData.businessRules) {
            (Object.values(enterpriseData.businessRules as Record<string, unknown>) as Record<string, unknown>[]).forEach((productRule: Record<string, unknown>) => {
                (Object.entries(productRule.sizes as Record<string, unknown>) as [string, Record<string, unknown>][]).forEach(([sizeKey, sizeRule]: [string, Record<string, unknown>]) => {
                    (Object.entries(sizeRule.profiles as Record<string, unknown>) as [string, Record<string, unknown>][]).forEach(([, profileData]: [string, Record<string, unknown>]) => {
        ...
        return {
            ...
            products: (enterpriseData.products as Record<string, { name: string; totalOrderQuantity: number; totalQuantity: number; averageRatio: number; minRatio: number; maxRatio: number; patternCount: number; sizes: string[]; profiles: string[]; multiProfileCount: number; singleProfileCount: number; patterns: ProfilePattern[] }>) || {},
            combinations: combinations as unknown as Record<string, { product: string; size: string; profile: string; averageRatio: number; averageQuantity: number; averageOrderQuantity: number; patternCount: number; patterns: ProfilePattern[] }>,
            multiProfileAnalysis: {
    ...
    private buildProductProfileDatabase(): void {
        ...
        const productSizeGroups = new Map<string, Map<string, Record<string, unknown>>>();
        ...
        productSizeGroups.forEach((profileGroup, productSizeKey) => {
            ...
            const sortedProfiles = Array.from(profileGroup.entries())
                .sort(([,a], [,b]) => ((b.averageRatio as number) || 0) - ((a.averageRatio as number) || 0));
            ...
            sortedProfiles.forEach(([profileName, profileData]) => {
                ...
                const exampleQuantity = Math.round(((profileData.averageRatio as number) || 0) * 100);
                ...
                console.log(`[INFO] ${product} | ${size} | ${profileName || '(Empty)'}: ${exampleQuantity} units (${length}mm) - ratio: ${((profileData.averageRatio as number) || 0).toFixed(2)}x [${(profileData.patternCount as number) || 0} samples]`);
            });
            ...
            if (profiles.length > 0) {
                const productProfileData: ProductProfileData = {
                    ...
                    patterns: Array.from(profileGroup.values()).flatMap(p => (p.patterns as ProfilePattern[]) || [])
                };

                this.productProfileDatabase.set(productSizeKey, [productProfileData]);
                console.log(`[INFO] Product: ${product} | ${size}: ${profiles.length} profiles, total ${totalExampleQuantity} pieces`);
            }
        });

        console.log(`[INFO] Database created: ${this.productProfileDatabase.size} combinations`);
        ...
        topCombinations.forEach((combo, index) => {
            if (combo.data) {
                console.log(`[INFO]    ${index + 1}. ${combo.data.productName} | ${combo.data.size}: ${combo.data.profiles.length} profiles, ${combo.data.patterns.length} samples`);
                combo.data.profiles.forEach((profile, pIndex) => {
                    ...
                    console.log(`[INFO]       ${pIndex + 1}. ${profile.profileName || '(Empty)'}: ${profile.quantity} units (${(ratio * 100).toFixed(1)}%)`);
                });
            }
        });
    }
    ...
    private findEnterpriseProfileData(productName: string, size: string, profile: string, profileIndex: number): { ratio: number; confidence: number } | null {
        ...
        console.log(`Veri arama: ${productName}|${size}|${profile} (index: ${profileIndex})`);
        ...
        if (profileIndex !== undefined) {
            ...
            if (productName === 'INCE HELEZON' && size === '24"X36"') {
                if (profileIndex === 0) {
                    ...
                    console.log(`[INFO] Special product logic: Profile ${profileIndex} 1.00x`);
        ...
        if (profileKeys.length > 0) {
            ...
            if (firstKey && this.excelData.combinations[firstKey]) {
                ...
                console.log(`[INFO] Profile match: ${comboData.averageRatio.toFixed(2)}x`);
                return {
                    ratio: comboData.averageRatio,
                    confidence: Math.min(0.95, Math.max(0.1, comboData.patternCount / 10))
                };
    ...
    private createDefaultProfileDatabase(): void {
        ...
        console.log('[INFO] Profile database initialized empty - will be populated from user configurations');
    ...
    public calculateQuantity(
        ...
        try {
            console.log(`[INFO] Business logic: Product=${productName || 'AUTO'}, Order=${siparisAdedi}, Size=${size}, Profile=${profile}, ProfileIndex=${profileIndex}`);

            // Ürün adını belirle (önce parametre, sonra otomatik tespit)
            const detectedProduct = this.detectProductFromProfile(profile, size) || productName || 'INCE HELEZON';
            ...
            if (enterpriseResult) {
                ...
                console.log(`[INFO] ${detectedProduct} → ${size} → ${profile} → ${enterpriseResult.ratio.toFixed(2)}x → ${calculatedAdet} units`);
            ...
            if (profileData && profileData[0]) {
                ...
                if (profiles && profiles.length > 0) {
                    ...
                    if (firstProfile) {
                        ...
                        console.log(`[INFO] First profile default: ${firstProfile.profileName} → ${ratio.toFixed(2)}x → ${calculatedAdet} units`);
            ...
            if (totalProfiles > 1) {
                ...
                console.log(`[INFO] General multi-profile rule: Profile ${profileIndex + 1}/${totalProfiles} = ${calculatedAdet}`);
                return {
                    adet: calculatedAdet,
                    confidence: 0.8,
            ...
            if (fallbackProductSizeKey && this.productProfileDatabase.has(fallbackProductSizeKey)) {
                ...
                if (profileData && profileData[0]) {
                    ...
                    if (targetProfile && profileData[0].totalPieces > 0) {
                        ...
                        console.log(`[INFO] Profile-based calculation: ${profile} (${targetProfile.length}mm) = ${calculatedAdet} units`);

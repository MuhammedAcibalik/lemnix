...
export class ProductionPlanService {
  ...
  async uploadProductionPlan(
    ...
    try {
      ...
      return {
        success: false,
        errors: [
          `Üretim planı oluşturulurken hata oluştu: ${error instanceof Error ? error.message : "Bilinmeyen hata"}`,
        ],
      };
    }
  }
        ...
        try {
          return {
            ...
            ad: itemWithEncrypted.encryptedAd
              ...
              : encryptionService.isEncrypted(item.ad)
                ? encryptionService.decryptString(item.ad)
                : item.ad,
            siparisVeren: itemWithEncrypted.encryptedSiparisVeren
              ...
              : encryptionService.isEncrypted(item.siparisVeren)
                ? encryptionService.decryptString(item.siparisVeren)
                : item.siparisVeren,
            musteriNo: itemWithEncrypted.encryptedMusteriNo
              ...
              : encryptionService.isEncrypted(item.musteriNo)
                ? encryptionService.decryptString(item.musteriNo)
                : item.musteriNo,
            musteriKalemi: itemWithEncrypted.encryptedMusteriKalemi
              ...
              : encryptionService.isEncrypted(item.musteriKalemi)
                ? encryptionService.decryptString(item.musteriKalemi)
                : item.musteriKalemi,
            siparis: itemWithEncrypted.encryptedSiparis
              ...
              : encryptionService.isEncrypted(item.siparis)
                ? encryptionService.decryptString(item.siparis)
                : item.siparis,
            malzemeNo: itemWithEncrypted.encryptedMalzemeNo
              ...
              : encryptionService.isEncrypted(item.malzemeNo)
                ? encryptionService.decryptString(item.malzemeNo)
                : item.malzemeNo,
            malzemeKisaMetni: itemWithEncrypted.encryptedMalzemeKisaMetni
              ...
              : encryptionService.isEncrypted(item.malzemeKisaMetni)
                ? encryptionService.decryptString(item.malzemeKisaMetni)
  ...
  async getProductionPlanMetrics(
    ...
    plans.forEach((plan) => {
      plan.items.forEach((item) => {
        priorityDistribution[item.oncelik] =
          (priorityDistribution[item.oncelik] || 0) + 1;
      });
    });
  ...
  async getProductionPlanItemsByWorkOrder(
    ...
    return await this.prisma.productionPlanItem.findMany({
      ...
      orderBy: [{ plan: { weekNumber: "desc" } }, { oncelik: "asc" }],
    });
  }
...
function decryptProductionPlanItem(

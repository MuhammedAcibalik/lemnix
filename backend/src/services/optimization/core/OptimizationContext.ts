...
export class OptimizationContext {
  ...
  constructor(params: {
    ...
    if (!params.objectives || params.objectives.length === 0) {
      throw new Error('OptimizationContext: at least one objective is required');
    }
    
    const weightSum = params.objectives.reduce((sum, obj) => sum + obj.weight, 0);
    ...
    this.stockLengths = Object.freeze(
      params.materialStockLengths?.length
        ? params.materialStockLengths.map(s => s.stockLength).sort((a, b) => a - b)
        : [6100]
    );
    ...
    this.nsgaParams = params.nsgaParams ? Object.freeze({ ...params.nsgaParams }) : undefined;
    this.timeModel = params.timeModel ? Object.freeze({ ...params.timeModel }) : undefined;
  }

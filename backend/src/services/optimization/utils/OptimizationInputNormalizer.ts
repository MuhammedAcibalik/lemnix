...
export class OptimizationInputNormalizer {
   ...
   */
  buildDemandFromItems(items: ReadonlyArray<OptimizationItem>): Map<number, number> {
    ...
    for (const item of items) {
      ...
      if (Number.isNaN(quantity)) {
        this.logger.warn('[Normalizer] Skipping item with invalid quantity:', { item });
        continue;
      }
  ...
  buildDemandFromRawRows(
    ...
    for (const row of rows) {
      ...
      if (Number.isNaN(qty)) {
        this.logger.warn('[Normalizer] Skipping row with invalid quantity:', { row });
        continue;
      }

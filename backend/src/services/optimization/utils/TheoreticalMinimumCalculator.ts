...
export class TheoreticalMinimumCalculator {
  ...
  static calculateMinimumStock(
    ...
    const totalWithOverhead = totalRequired + 
    ...
    const optimal = breakdown.reduce((best, curr) => 
      curr.count < best.count || (curr.count === best.count && curr.coverage > best.coverage)
        ? curr : best
    );
  ...
  static validateResult(
    ...
    const overheadPercentage = (actualCuts - theoreticalMin.minStockCount) / theoreticalMin.minStockCount;

...
export interface PatternSolution {
   ...
   */
  readonly patternCounts: ReadonlyArray<{ readonly patternId: string; readonly count: number }>;
...
export function createPatternId(
  ...
  const itemsStr = sortedLengths.map(length => `${length}x${items.get(length)}`).join('_');
...
export function dominates(patternA: Pattern, patternB: Pattern): boolean {
  ...
  const allLengths = new Set([...patternA.items.keys(), ...patternB.items.keys()]);
  ...
  return patternA.waste < patternB.waste || 

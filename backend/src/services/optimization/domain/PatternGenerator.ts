  ...
  private generatePatternsDFS(
    ...
    if (remaining < minItemLength || itemIndex >= items.length) {
      ...
      if (currentItems.size > 0) {
        const pattern = this.createPatternFromItems(
          currentItems,
          remaining,
          stockLength,
          kerfWidth,
    ...
    const maxCount =
      kerfWidth === 0
        ...
        : Math.min(
            item.maxCount,
            Math.floor(
              (remainingSpace + kerfWidth) / (item.length + kerfWidth),
            ),
          );
    ...
    for (let count = 0; count <= maxCount; count++) {
      ...
      const newKerf =
        kerfWidth > 0 && newSegments > 0 ? (newSegments - 1) * kerfWidth : 0;

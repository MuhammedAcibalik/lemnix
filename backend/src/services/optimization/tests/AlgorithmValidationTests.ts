...
export class AlgorithmValidationTests {
  ...
  public static testRemainingLengthCalculation(): void {
    ...
    for (const testCase of testCases) {
      const usedLength =
        testCase.startSafety + testCase.itemLength + (testCase.kerfWidth || 0);
      const remainingLength =
        testCase.stockLength - usedLength - testCase.endSafety;

      console.log(`✅ ${testCase.name}:`);
      console.log(
        `   Used: ${usedLength}mm (expected: ${testCase.expectedUsed}mm)`,
      );
      console.log(
        `   Remaining: ${remainingLength}mm (expected: ${testCase.expectedRemaining}mm)`,
      );
      console.log(
        `   Accounting check: ${usedLength + remainingLength + testCase.endSafety} = ${testCase.stockLength} ✅`,
      );
    }
  }
  ...
  public static testStockSelectionOptimization(): void {
    ...
    for (const item of testItems) {
      ...
      console.log(
        `   Selected: ${selected}mm (expected: ${item.expectedOptimal}mm)`,
      );
      console.log(
        `   Correct: ${selected === item.expectedOptimal ? "✅" : "❌"}`,
      );
    }
  }
      ...
      try {
        ...
        for (const cut of result.cuts) {
          ...
          if (difference > 0.01) {
            accountingErrors++;
            console.log(
              `❌ Accounting error in cut ${cut.id}: ${cut.usedLength} + ${cut.remainingLength} ≠ ${cut.stockLength} (diff: ${difference}mm)`,
            );
          }
        }

        console.log(
          `✅ ${algorithm.constructor.name}: ${accountingErrors === 0 ? "All cuts balanced" : `${accountingErrors} accounting errors`}`,
        );
  ...
  public static async testPerformanceComparison(): Promise<void> {
    ...
    for (const scenario of testScenarios) {
      ...
      for (const algorithm of algorithms) {
        try {
          const result = await algorithm.optimize(context);
          results[algorithm.constructor.name] =

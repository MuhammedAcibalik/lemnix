...
export interface DataProvider {
  getOptimizationItems(params?: {
    workOrderIds?: string[];
    profileTypes?: string[];
  }): Promise<ReadonlyArray<OptimizationItem>>;
...
export class AdvancedOptimizationService {
  ...
  constructor(logger?: ILogger, prisma?: PrismaClient) {
    ...
    this.profileResolver = prisma
      ? new ProfileResolverService(prisma, this.logger)
      : null;
    ...
    this.logger.info(
      "AdvancedOptimizationService initialized (v5.0.0 + Hybrid Algorithm Support + Profile Management)",
    );
  }
  ...
  public async optimize(
    ...
    try {
      // Validate algorithm
      if (
        !this.algorithmFactory.isRegistered(
          this.mapLabelToEnum(params.algorithm),
        )
      ) {
        throw new Error(
          `Algorithm '${params.algorithm}' not registered. Available: ${this.algorithmFactory.getRegisteredAlgorithms().join(", ")}`,
        );
      }
      ...
      if (
        ...
        profileParams?.workOrderId &&
        profileParams?.profileType
      ) {
        const profileResult =
          await this.profileResolver.resolveProfileForWorkOrder({
        ...
        if (profileResult) {
          ...
          this.logger.warn(
            "[Optimization] No profile found, using default stock lengths",
            {
              workOrderId: profileParams.workOrderId,
              profileType: profileParams.profileType,
            },
      ...
      const enhancedMetadata = profileInfo
        ? {
            ...
            profile: {
              ...
              stockLengths:
                resolvedStockLengths?.map((sl) => sl.stockLength) ?? [],
            },
          }
        : result.metadata;
      ...
      this.logger.info("Optimization completed", {
        ...
        profileUsed: profileInfo
          ? `${profileInfo.profileCode} (${profileInfo.source})`
          : "none",
      });
  ...
  private validateResult(
    ...
    const totalSegments = result.cuts.reduce(
      (sum, cut) => sum + cut.segmentCount,
      0,
    );
    const expectedSegments = items.reduce(
    ...
    if (totalSegments < expectedSegments) {
      this.logger.error(
        `Missing items: ${expectedSegments - totalSegments} items not placed`,
        {
          totalSegments,
          expectedSegments,
    ...
    if (totalSegments > expectedSegments) {
      this.logger.warn(
        `Overproduction detected: +${totalSegments - expectedSegments} segments`,
        {
          totalSegments,
          expectedSegments,
    ...
    if (totalCuts > theoreticalMin.minStockCount * 2) {
      this.logger.warn(
        `Algorithm used ${totalCuts} cuts, theoretical minimum is ${theoreticalMin.minStockCount} (${Math.round((totalCuts / theoreticalMin.minStockCount - 1) * 100)}% over)`,
        {
          actualCuts: totalCuts,
          theoreticalMin: theoreticalMin.minStockCount,
          overheadPercentage: Math.round(
            (totalCuts / theoreticalMin.minStockCount - 1) * 100,
          ),
        },
      );
    }
    ...
    const totalUsed = result.cuts.reduce(
      (sum, cut) => sum + cut.stockLength,
      0,
    );
    if (totalUsed > theoreticalMin.totalRequired * 1.5) {
      this.logger.warn(
        `Total stock used ${totalUsed}mm is ${Math.round((totalUsed / theoreticalMin.totalRequired - 1) * 100)}% over theoretical ${theoreticalMin.totalRequired}mm`,
        {
          totalUsed,
          theoreticalRequired: theoreticalMin.totalRequired,
          overheadPercentage: Math.round(
            (totalUsed / theoreticalMin.totalRequired - 1) * 100,
          ),
        },
      );
    }
  ...
  public async optimizeWithMode(
    ...
    if (
      !params ||
      !params.objectives ||
      !Array.isArray(params.objectives) ||
    ...
    try {
      ...
      this.logger.debug(
        "AdvancedOptimizationService DEBUG - Context creation:",
        {
          itemsCount: items.length,
          materialStockLengths: resolvedStockLengths?.map((s) => ({
   ...
   */
  private getAlgorithmDisplayName(
    algorithm: string,
    mode: AlgorithmMode,
  ): string {

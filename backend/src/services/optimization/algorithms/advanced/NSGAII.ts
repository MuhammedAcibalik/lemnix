...
export class NSGAIIAlgorithm extends GeneticAlgorithm {
  public override readonly name: OptimizationAlgorithm =
    OptimizationAlgorithm.NSGA_II; // Multi-objective Pareto optimization
   ...
   */
  private static readonly DEDUP_EPS = Math.max(
    1e-6,
    10 * NSGAIIAlgorithm.DOM_EPS,
  );
  ...
  private async tryGPUParetoOptimization(
    ...
    try {
      ...
      const gpuFrontAsIndividuals: ParetoIndividual[] = [
        {
          ...
          objectives: this.extractObjectivesFromSequence(
            sequenceItems,
            context,
          ),
          rank: 0,
      ...
      const hypervolume =
        gpuFrontAsIndividuals.length > 0
      ...
      this.logger.info(
        "GPU NSGA-II completed successfully with local metric calculation",
        {
          frontSize: 1,
  ...
  private extractObjectivesFromSequence(
    ...
    try {
      ...
      if (!isValidObjectives(objectives)) {
        this.logger.warn(
          "NSGA-II: Invalid objectives generated, using defaults",
          { objectives },
        );
        return {
          waste: 0,
  ...
  private async optimizeMultiObjectiveOnCPU(
    ...
    const convergenceThreshold =
      context.nsgaParams?.convergenceThreshold ?? 1e-5;
    ...
    const initialSequences = this.initializePopulation(
      expanded,
      populationSize,
    );

    let population: ParetoIndividual[] = initialSequences.map(
      (seq: OptimizationItem[]) => {
    ...
    for (let gen = 0; gen < generations; gen++) {
      ...
      const parents = this.crowdedTournamentSelection(
        fronts.flat(),
        populationSize,
      );
      ...
      if (gen % 20 === 0) {
        this.logger.info(`NSGA-II Gen ${gen}/${generations}`, {
          ...
          avgEfficiency: this.average(
            firstFront.map((i) => i.objectives.efficiency),
          ),
          hv_tracking_fixed_ref:
            fmtStr(hypervolumeHistory[hypervolumeHistory.length - 1], 4) ??
            "N/A",
    ...
    if (paretoFront.length === 0) {
      this.logger.error(
        "NSGA-II: Final Pareto front is empty - population converged incorrectly",
        {
          populationSize: population.length,
          generations,
          convergenceReason,
      ...
      if (paretoFront.length === 0) {
        throw new Error(
          "NSGA-II: Both Pareto front and population are empty. Cannot produce results.",
        );
      }

      this.logger.warn(
        "NSGA-II: Using entire population as fallback Pareto front",
        {
    ...
    const hypervolume = this.safeHypervolume(
      sortedFront,
      this.hvReferenceIdeal || { waste: 1, cost: 1, efficiency: 0, time: 1 },
    );
    ...
    if (
      hypervolume !== null &&
      (!Number.isFinite(hypervolume) || hypervolume < 0)
    ) {
      this.logger.warn("NSGA-II: Invalid hypervolume", { hypervolume });
    }

    if (spread !== null && (spread < 0 || spread > 2)) {
      this.logger.warn("NSGA-II: Spread metric out of expected range", {
        spread,
      });
    }
    ...
    return {
      ...
      spread: safeSpread,
      recommendedSolution:
        kneePoint?.result || this.createDefaultResult(context),
      frontSize: paretoFront.length,
    };
  }
  ...
  private dominates(a: ObjectiveValues, b: ObjectiveValues): boolean {
    ...
    const allBetterOrEqual =
      aWasteBetter && aCostBetter && aTimeBetter && aEffBetter;
   ...
   */
  private deduplicateByObjectives(
    front: ParetoIndividual[],
  ): ParetoIndividual[] {
   ...
   */
  private selectOrderingAxis(
    normalized: ParetoIndividual[],
  ): keyof ObjectiveValues {
  ...
  private assignCrowdingDistance(front: ParetoIndividual[]): void {
    ...
    NSGAIIAlgorithm.OBJECTIVE_KEYS.forEach((obj) => {
      ...
      const sorted = [...front].sort(
        (a, b) => a.objectives[obj] - b.objectives[obj],
      );
      ...
      for (let i = 1; i < n - 1; i++) {
        sorted[i].crowdingDistance +=
          (sorted[i + 1].objectives[obj] - sorted[i - 1].objectives[obj]) /
          objRange;
      }
    });
  }
  ...
  private crowdedTournamentSelection(
    ...
    for (let i = 0; i < count; i++) {
      ...
      const idx1 = Math.floor(this.random() * population.length);
      ...
      if (!i1 || !i2) {
        ...
        if (fallback) {
          ...
          this.logger.error(
            "NSGA-II: No valid individuals in population for tournament",
            {
              populationSize: population.length,
              iteration: i,
            },
    ...
    if (selected.length < count && population.length > 0) {
      this.logger.warn(
        "NSGA-II: Tournament selection undersized, filling with random individuals",
        {
          expected: count,
      ...
      while (selected.length < count) {
        ...
        if (fallback) {
          ...
          this.logger.error(
            "NSGA-II: Cannot fill tournament selection, no valid individuals",
          );
          break;
        }
      }
  ...
  private crowdedCompare(
    ...
    if (
      Math.abs(a.crowdingDistance - b.crowdingDistance) >
      NSGAIIAlgorithm.NORMALIZE_EPS
    ) {
   ...
   */
  private estimateTimeIfMissing(
    result: AdvancedOptimizationResult,
    context: OptimizationContext,
  ): number {
    const cutCount = result.cuts?.length ?? 0;
    const segmentCount =
      result.cuts?.reduce((sum, cut) => sum + (cut.segments?.length ?? 0), 0) ??
      0;
   ...
   */
  private dedupVectors(
    vectors: number[][],
    eps = NSGAIIAlgorithm.DEDUP_EPS,
  ): number[][] {
    const seen = new Set<string>();
   ...
   */
  private enforceReferenceDominance(
    points: number[][],
    reference: number[],
  ): number[][] {
  ...
  private calculateHypervolumeForTracking(front: ParetoIndividual[]): number {
    ...
    if (bounded.length === 0) {
      this.logger.debug(
        "NSGA-II: All points beyond HV reference, using cached value",
        {
  ...
  private calculateHypervolume(front: ParetoIndividual[]): number {
    if (front.length === 0) {
      throw new Error(
        "NSGA-II: Cannot calculate hypervolume on empty Pareto front",
      );
    }
    ...
    const rawPoints = front.map((ind) =>
      this.toMinNormalizedVector(ind.objectives, ideal, nadir),
    );
    ...
    const allDominated = refPoint.every(
      (val, i) =>
    ...
    if (!allDominated) {
      this.logger.warn(
        "NSGA-II: Reference point adjustment in normalized space",
        {
          refPoint: [...refPoint],
      ...
      this.logger.debug(
        "NSGA-II: Adjusted reference point in normalized space",
        {
          original: [...refPoint],
  ...
  private toMinNormalizedVector(
    ...
    return [
      ...
      this.normalizeValue(
        ideal.efficiency - obj.efficiency,
        0,
        ideal.efficiency - nadir.efficiency,
      ),
    ];
  }
  ...
  private calculateSpread(front: ParetoIndividual[]): number {
    ...
    const sumDeviations = distances.reduce(
      (sum, d) => sum + Math.abs(d - dAvg),
      0,
    );
  ...
  private normalizeObjectives(front: ParetoIndividual[]): ParetoIndividual[] {
    ...
    return front.map((ind) => ({
      ...ind,
      objectives: {
        waste: this.normalizeValue(
          ind.objectives.waste,
          ideal.waste,
          nadir.waste,
        ),
        cost: this.normalizeValue(ind.objectives.cost, ideal.cost, nadir.cost),
        efficiency: this.normalizeValue(
          ind.objectives.efficiency,
          nadir.efficiency,
          ideal.efficiency,
        ), // Reversed for maximize
   ...
   */
  private calculateBoundaryDistances(normalized: ParetoIndividual[]): {
    dF: number;
    dL: number;
  } {
    let dF = 0;
   ...
   */
  private calculateConsecutiveDistances(
    normalized: ParetoIndividual[],
  ): number[] {
    ...
    const sorted = [...normalized].sort(
      (a, b) => a.objectives[orderAxis] - b.objectives[orderAxis],
    );
  ...
  private selectKneePoint(front: ParetoIndividual[]): ParetoIndividual {
    if (front.length === 0)
      throw new Error(
        "NSGA-II: Cannot select knee point from empty Pareto front",
      );
  ...
  private calculateAngleScore(
    ...
    if (
      mag1 < NSGAIIAlgorithm.NORMALIZE_EPS ||
      mag2 < NSGAIIAlgorithm.NORMALIZE_EPS
    )
      return 0;
   ...
   */
  private vectorBetweenNormalized(
  ...
  private euclideanDistance(a: ObjectiveValues, b: ObjectiveValues): number {
    ...
    return Math.sqrt(
      dWaste * dWaste + dCost * dCost + dEff * dEff + dTime * dTime,
    );
  }
   ...
   */
  private safeHypervolume(
    front: ParetoIndividual[],
    refPoint: ObjectiveValues,
  ): number | null {
    if (!Array.isArray(front) || front.length === 0) {
      this.logger.warn(
        "NSGA-II: Empty Pareto front, returning null hypervolume",
      );
      return null;
    }
    ...
    if (!isValidObjectives(refPoint as unknown as Record<string, unknown>)) {
      this.logger.warn(
        "NSGA-II: Invalid reference point, returning null hypervolume",
        { refPoint },
      );
      return null;
    }
    ...
    for (const individual of front) {
      if (
        !isValidObjectives(
          individual.objectives as unknown as Record<string, unknown>,
        )
      ) {
        this.logger.warn(
          "NSGA-II: Invalid objectives in front member, returning null hypervolume",
          {
    ...
    try {
      ...
      this.logger.warn(
        "NSGA-II: Error calculating hypervolume, returning null",
        {
          error: error instanceof Error ? error.message : "Unknown error",
          frontSize: front.length,
        },
   ...
   */
  private safeHypervolumeAdvanced(
    front: Array<number[]>,
    refPoint: number[],
  ): number | null {
    ...
    try {
      ...
      this.logger.warn(
        "NSGA-II: Error calculating advanced hypervolume, returning null",
        {
          error: error instanceof Error ? error.message : "Unknown error",
        },
      );

...
import { AdvancedOptimizationResult, StockSummary, EnhancedConstraints } from '../types';
...
export class BFDAlgorithm extends BaseAlgorithm {
   ...
   */
  private prepareSortedItems(context: OptimizationContext): ReadonlyArray<OptimizationItem> {
  ...
  private executeBFDAlgorithm(
    ...
    console.log(`\n🔥🔥🔥 [BFD] executeBFDAlgorithm CALLED! 🔥🔥🔥`);
    console.log(`[BFD] Theoretical minimum: ${theoreticalMin.minStockCount} stocks`);
    console.log(`[BFD] Total required: ${theoreticalMin.totalRequired}mm`);
    console.log(`[BFD] Kerf width: ${context.constraints.kerfWidth}`);
    console.log(`[BFD] Stock lengths: ${context.stockLengths}`);
    console.log(`[BFD] Items:`, sortedItems.map(i => `${i.length}mm x${i.quantity || 1}`));
    
    this.logger.info(`[BFD] Theoretical minimum: ${theoreticalMin.minStockCount} stocks for ${theoreticalMin.totalRequired}mm total`, {
    ...
    console.log(`[BFD] ✅ Calling executeDPOptimization...`);
    this.logger.info(`[BFD] Using DP optimization (kerf=${context.constraints.kerfWidth})`);
   ...
   */
  private initializeMultiStockBins(stockLengths: ReadonlyArray<number>): MultiStockBins {
  ...
  private optimizeRemainingSpace(
    ...
    for (const item of sortedRemaining) {
      ...
      if (StockCalculator.canFitItem(
        ...
        this.logger.debug(`[BFDAlgorithm] ✅ Filled remaining space with ${item.length}mm item:`, {
  ...
  private placeItemOptimally(
    ...
    while (remainingQuantity > 0) {
      ...
      if (bestFit !== null) {
        ...
        const piecesThatFit = this.calculatePiecesThatFit(item, bestFit.cut, context);
        const piecesToPlace = Math.min(remainingQuantity, piecesThatFit);
        
        this.placeItemInExistingCut({ ...item, quantity: piecesToPlace }, bestFit, bins, context);
        remainingQuantity -= piecesToPlace;
      } else {
        // Create new cut and place as many pieces as possible
        const piecesToPlace = remainingQuantity;
        this.placeItemInNewCut({ ...item, quantity: piecesToPlace }, bins, context);
        remainingQuantity = 0;
      }
    }
  }
  ...
  private findBestFitLocation(
    ...
    for (const [stockLength, cuts] of bins.binsByLength) {
      const localBestFit = this.findBestFitInStockLength(item, cuts, stockLength, context);
  ...
  private findBestFitInStockLength(
    ...
    for (const cut of cuts) {
      ...
      if (fitResult !== null) {
        ...
        if (bestFit === null || fitResult.adjustedWaste < bestFit.adjustedWaste) {
          bestFit = fitResult;
        } else if (Math.abs(fitResult.adjustedWaste - bestFit.adjustedWaste) < 0.01) {
          // Tie-breaker: prefer higher future opportunity
          if (fitResult.futureOpportunityScore > bestFit.futureOpportunityScore) {
            bestFit = fitResult;
          }
        }
      }
  ...
  private evaluateFitInCut(
    ...
    if (cut.remainingLength >= totalNeeded) {
      ...
      const willCreateFragment = wasteAfterFit > 0 && 
                                 wasteAfterFit < context.constraints.minScrapLength;
      ...
      const futureScore = this.calculateFutureOpportunityScore(wasteAfterFit, context);
  ...
  private placeItemInExistingCut(
    ...
    let updatedCut = this.addSegmentToCut(bestFit.cut, item, kerfNeeded, context);
    
    // Optimize remaining space with different item lengths
    updatedCut = this.optimizeRemainingSpace(updatedCut, [...this.upcomingItems], context);
  ...
  private placeItemInNewCut(
    ...
    const selectedStockLength = this.selectOptimalStockLength(item, bins, context);
  ...
  private selectOptimalStockLength(
    ...
    for (const stockLength of context.stockLengths) {
      ...
      const usedLength = context.constraints.startSafety + 
   ...
   */
  private calculateStockUsageStatistics(bins: MultiStockBins): StockLengthUsage[] {
   ...
   */
  private checkUniformUsage(usageStats: ReadonlyArray<StockLengthUsage>): boolean {
  ...
  private createNewStock(
    ...
    const usableLength = stockLength - constraints.startSafety - constraints.endSafety;
  ...
  private validateStockAccounting(
    ...
    if (!StockCalculator.validateStockAccounting(
      finalUsedLength,
      finalRemaining,
      cut.stockLength
    )) {
  ...
  private buildOptimizationResult(
    ...
    const timing = this.calculateTimingMetrics(aggregates.totalSegments, cuts.length);
    const quality = this.calculateQualityMetrics(efficiency, aggregates.totalWaste);
    ...
    return {
      ...
      totalSafetyReserve: cuts.length * (
  ...
  private calculateQualityMetrics(
    ...
    const qualityScore = MetricsCalculator.calculateQualityScore(efficiency, totalWaste);
    const wastePercentage = WasteAnalyzer.calculateWastePercentage(totalWaste, 0);
    ...
    return {
      ...
      confidence: MetricsCalculator.calculateConfidence(efficiency, totalWaste, 0)
    };
  }
   ...
   */
  private calculateAverageCutsPerStock(totalSegments: number, stockCount: number): number {
    if (stockCount === 0) {
      return 0;
    }
  ...
  private executeDPOptimization(
    sortedItems: ReadonlyArray<OptimizationItem>,
    context: OptimizationContext,
    theoreticalMin: { readonly minStockCount: number; readonly totalRequired: number; readonly recommendedStockLength: number }
  ): Cut[] {
    this.logger.info(`[BFD] Starting DP optimization (kerf=${context.constraints.kerfWidth})`);
    ...
    console.log(`\n========== BFD PROBLEM ANALYSIS ==========`);
    console.log(`Unique lengths: ${uniqueLengths}`);
    console.log(`Total demand: ${totalDemand}`);
    console.log(`Item groups:`, itemGroups.map(g => `${g.length}mm x${g.quantity}`).join(', '));
    console.log(`Stock lengths:`, stockLengths);
    console.log(`==========================================\n`);
    ...
    if (uniqueLengths > 15 || totalDemand > 1000) {
      console.log(`🔴 [BFD] USING TRADITIONAL BFD (problem too large)`);
      this.logger.info(`[BFD] Large problem (${uniqueLengths} lengths, ${totalDemand} items) - using traditional BFD for guaranteed low waste`);
    ...
    console.log(`🟢 [BFD] USING PATTERN-BASED OPTIMIZATION`);

    
    const usePatternLimit = uniqueLengths > 10;
    if (usePatternLimit) {
      this.logger.info(`[BFD] Medium problem (${uniqueLengths} lengths) - using limited pattern generation`);
    }
    ...
    if (patterns.length === 0) {
      this.logger.warn('[BFD] No valid patterns found, falling back to traditional BFD');
      return this.executeTraditionalBFD(sortedItems, context);
    }
    ...
    try {
      ...
      this.logger.info(`[BFD] DP optimization completed:`, {
        ...
        efficiency: cuts.length > 0 ? ((cuts.reduce((sum, c) => sum + c.usedLength, 0) / cuts.reduce((sum, c) => sum + c.stockLength, 0)) * 100).toFixed(2) : 0
      });
      ...
      console.log(`\n⚠️⚠️⚠️ PATTERN-BASED FAILED! FALLBACK TO TRADITIONAL BFD ⚠️⚠️⚠️`);
      console.log(`Error: ${error instanceof Error ? error.message : String(error)}`);
      console.log(`Patterns generated: ${patterns.length}`);
      console.log(`Unique lengths: ${itemGroups.length}`);
      console.log(`==========================================\n`);
      
      this.logger.warn('[BFD] Pattern-based approach failed, falling back to traditional BFD:', {
   ...
   */
  private convertBinsToCuts(bins: MultiStockBins, context: OptimizationContext): Cut[] {
    const cuts: Cut[] = [];
  ...
  private generateCuttingPatterns(
    ...
    for (const stockLength of sortedStockLengths) {
      ...
      if (maxPatterns && patterns.length >= maxPatterns) {
        this.logger.warn(`[BFD] Pattern limit reached (${patterns.length}/${maxPatterns}) - stopping generation`);
        break;
      }
    }
    ...
    this.logger.info(`[BFD] Pattern filtering applied:`, {
      ...
      reductionPct: ((beforeCount - filtered.length) / beforeCount * 100).toFixed(1) + '%'
    });
  ...
  private generateCombinations(
    ...
    const actualMaxCount = kerfWidth === 0
  ...
  private validateDemandFulfillment(
    ...
    cuts.forEach((cut, cutIndex) => {
      ...
      cut.segments.forEach((seg, segIndex) => {
        ...
        this.logger.debug(`[BFD] 🔍 Added segment ${segIndex}: ${seg.length}mm`, {
  ...
  private findExactDemandSolution(
    ...
    if (!finalValidation.isValid) {
      this.logger.error(`[BFD] ❌ Priority search failed to meet exact demand:`, {
  ...
  private findBestFittingPattern(
    ...
    for (const pattern of patterns) {
      ...
      const itemsInPattern = Array.from(pattern.pattern.values()).reduce((sum, c) => sum + c, 0);
      const wastePerItem = itemsInPattern > 0 ? pattern.waste / itemsInPattern : Infinity;
      const wasteScore = -wastePerItem * 100; // Penalize waste per item
      
      // Stock utilization bonus: reward efficient use of stock
      const utilization = (pattern.used + pattern.waste) > 0
  ...
  private validateExactDemand(
    ...
    for (const group of itemGroups) {
      ...
      if (actual < required) {
        ...
        this.logger.warn(`[BFD] Over-produced ${group.length}mm: ${actual - required} extra (limit: ${maxExtraPerPiece})`);
      }
    }

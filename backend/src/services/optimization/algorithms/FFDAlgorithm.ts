...
export class FFDAlgorithm extends BaseAlgorithm {
  ...
  public async optimize(
    ...
    this.logger.info(
      `[FFD] Using DP optimization (kerf=${context.constraints.kerfWidth})`,
    );
  ...
  private runFFD(
    ...
    this.logger.debug(
      "[FFDAlgorithm] Multi-stock length bin packing initialized",
      {
    ...
    for (const item of sortedItems) {
      ...
      for (const [stockLength, bins] of binsByStockLength) {
        for (const cut of bins) {
          ...
          if (cut.remainingLength >= totalNeeded) {
            ...
            placed = true;
            this.logger.debug(
              `[FFDAlgorithm] Placed item in existing ${stockLength}mm cut`,
              {
      ...
      if (!placed) {
        ...
        const stockLengthUsage = Array.from(binsByStockLength.entries()).map(
          ([length, bins]) => ({
            length,
            count: bins.length,
        ...
        const allSameUsage = stockLengthUsage.every(
          (usage) => usage.count === stockLengthUsage[0]!.count,
        );
        ...
        const newCut = this.createNewStock(
          selectedStockLength,
          binsByStockLength.get(selectedStockLength)!.length,
          context.constraints,
        );
        ...
        this.logger.debug(
          `[FFDAlgorithm] Created new ${selectedStockLength}mm cut`,
          {
            itemLength: item.length,
    ...
    this.logger.debug(
      "[FFDAlgorithm] Multi-stock length bin packing completed",
      {
        totalCuts: allCuts.length,
        cutsByStockLength: Array.from(binsByStockLength.entries()).map(
          ([length, cuts]) => ({
            stockLength: length,
   ...
   * NOTE: This method does NOT mutate remainingItems - the caller should track used items separately
   */
  private optimizeRemainingSpace(
    cut: Cut,
    remainingItems: ReadonlyArray<OptimizationItem>,
    context: OptimizationContext,
  ): Cut {
    ...
    // Filter to only items that could potentially fit
    const sortedRemaining = [...remainingItems]
      .filter((item) => item.length < optimizedCut.remainingLength)
      .sort((a, b) => a.length - b.length);

    // Try to fit items without mutating the input array
    // Note: This is a greedy approach and doesn't remove items from the pool
    // The caller (runFFD) handles item consumption through the main loop
    for (const item of sortedRemaining) {
      ...
      if (
        ...
        this.logger.debug(
          `[FFDAlgorithm] ✅ Filled remaining space with ${item.length}mm item:`,
          {
        ...
        // Only fit one item at a time to avoid complexity
        break;
      }
    }
  ...
  private addSegmentToCut(
    ...
    if (process.env["NODE_ENV"] !== "production") {
      ...
      if (item.length > 100000) {
        this.logger.warn("Very large item length detected", {
          length: item.length,
        });
      }
    }
    ...
    this.logger.debug("[FFD] Created segment", {
    ...
    if (updatedCut.segmentCount !== updatedCut.segments.length) {
      throw new Error(
        `Invariant violation: segmentCount (${updatedCut.segmentCount}) !== segments.length (${updatedCut.segments.length})`,
      );
    }
  ...
  private generateCuttingPlan(
    ...
    const lengthMap = new Map<
      number,
      { count: number; profile?: string; itemId?: string }
    >();
   ...
   */
  private async executeDPOptimization(
    context: OptimizationContext,
  ): Promise<AdvancedOptimizationResult> {
    ...
    if (uniqueLengths > 15 || totalDemand > 1000) {
      this.logger.info(
        `[FFD] Large problem (${uniqueLengths} lengths, ${totalDemand} items) - using traditional FFD`,
      );
    ...
    if (patterns.length === 0) {
      this.logger.warn(
        "[FFD] No valid patterns found, falling back to traditional FFD",
      );
      return this.executeTraditionalFFD(context);
    }
    ...
    try {
      ...
      this.logger.warn(
        "[FFD] Pattern-based approach failed, falling back to traditional FFD:",
        {
   ...
   */
  private async executeTraditionalFFD(
    context: OptimizationContext,
  ): Promise<AdvancedOptimizationResult> {
  ...
  private generateCuttingPatterns(
    ...
    this.logger.info(
      `[FFD] Skipping Pareto filter to preserve all ${patterns.length} patterns`,
    );
    return patterns;
  }
  ...
  private validateDemandFulfillment(
    ...
    cuts.forEach((cut, cutIndex) => {
      ...
      cut.segments.forEach((seg, segIndex) => {
        ...
        this.logger.debug(
          `[FFD] 🔍 Added segment ${segIndex}: ${seg.length}mm`,
          {
  ...
  private findExactDemandSolution(
    ...
    if (!finalValidation.isValid) {
      this.logger.error(
        `[FFD] ❌ Exact demand validation failed:`,
  ...
  private convertSearchStateToSolution(
    ...
    this.logger.debug("[FFD] Converted search state to solution:", {
      ...
      patterns: solution
        .map(
          (s) => `${s.count}x(${this.generatePatternLabel(s.pattern.pattern)})`,
        )
        .join(", "),
    });
  ...
  private validateExactDemand(
    ...
    for (const group of itemGroups) {
      ...
      if (actual < required) {
        ...
        this.logger.warn(
          `[FFD] Over-produced ${group.length}mm: ${actual - required} extra (limit: ${maxExtraPerPiece})`,
        );
      }
    }

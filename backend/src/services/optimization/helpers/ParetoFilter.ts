 ...
 * @version 1.0.0
 * @architecture Domain helper - Pattern filtering strategy
 */

import type { ILogger } from "../../logger";

/**
 * Pattern interface for Pareto filtering
 */
interface Pattern {
  readonly stockId: string;
  readonly cuts: ReadonlyArray<{ readonly qty: number }>;
  readonly waste: number;
}

/**
 * Algorithm pattern format (used by BFD/Genetic)
 */
...
/**
 * Pareto Filter
 * Domain helper for pattern filtering using Pareto optimality principles
 */
export class ParetoFilter {
  ...
  private filterStock(
    ...
    const annotated = patterns.map((p) => ({
      pattern: p,
      totalPieces: p.cuts.reduce(
        (sum: number, c: { readonly qty: number }) => sum + c.qty,
        0,
      ),
    }));
    ...
    for (const annotatedPattern of annotated) {
      const isDominated = kept.some((keptPattern) => {
        const keptPieces = keptPattern.cuts.reduce(
          (sum: number, c: { readonly qty: number }) => sum + c.qty,
          0,
        );
        ...
        return (
          (keptPieces === patternPieces &&
            keptPattern.waste < annotatedPattern.pattern.waste) ||
          (keptPieces > patternPieces &&
            keptPattern.waste <= annotatedPattern.pattern.waste)
        );
      });
   ...
   * @returns Filtered patterns (Pareto optimal set)
   */
  paretoPerStockAlgorithm(
    patterns: ReadonlyArray<AlgorithmPattern>,
  ): AlgorithmPattern[] {
    if (patterns.length === 0) {
      return [];
    }
    ...
    for (const [stockLength, stockPatterns] of byStock.entries()) {
      const filteredStock = this.filterStockAlgorithm(
        stockLength,
        stockPatterns,
      );
      filtered.push(...filteredStock);
  ...
  private filterStockAlgorithm(
    ...
    const annotated = patterns.map((p) => ({
      pattern: p,
      totalPieces: Array.from(p.pattern.values()).reduce(
        (sum, count) => sum + count,
        0,
      ),
    }));
    ...
    for (const annotatedPattern of annotated) {
      const isDominated = kept.some((keptPattern) => {
        const keptPieces = Array.from(keptPattern.pattern.values()).reduce(
          (sum, count) => sum + count,
          0,
        );
        const patternPieces = annotatedPattern.totalPieces;

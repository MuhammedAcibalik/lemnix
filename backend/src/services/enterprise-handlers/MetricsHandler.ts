...
export class MetricsHandler extends BaseHandler implements IMetricsOperations {
  ...
  public async getMetrics(req: Request, res: Response): Promise<void> {
    try {
      ...
      this.sendSuccess(res, {
        ...
        optimizationMetrics: this.monitoringService.getOptimizationMetrics(limit),
  ...
  public async getAnalytics(req: Request, res: Response): Promise<void> {
    try {
      ...
      const optMetrics = this.monitoringService.getOptimizationMetrics(100) as OptimizationMetric[];
      const sysMetrics = this.monitoringService.getSystemMetrics(10) as SystemMetric[];
      
      const analytics = this.calculateAnalytics(optMetrics, sysMetrics, timeRange, algorithm);
  ...
  public async analyzePerformance(req: Request, res: Response): Promise<void> {
    try {
      ...
      let metrics = this.monitoringService.getOptimizationMetrics(1000) as OptimizationMetric[];
  ...
  private calculateAnalytics(
    ...
    if (optMetrics.length === 0) {
      return { timeRange, metrics: {}, algorithm: algorithm || 'N/A', generatedAt: new Date().toISOString() };
    }

    const filtered = algorithm ? optMetrics.filter(m => m.algorithm === algorithm) : optMetrics;
    const current = filtered[filtered.length - 1];
    const avg = (key: keyof OptimizationMetric): number => 
      filtered.reduce((s, m) => s + (Number(m[key]) || 0), 0) / (filtered.length || 1);

    return {
      timeRange,
      metrics: {
        efficiency: { current: current?.efficiency || 0, average: avg('efficiency'), trend: 'stable' as const },
        cost: { current: current?.executionTime || 0, average: avg('executionTime'), trend: 'stable' as const },
        waste: { current: current?.waste || 0, average: avg('waste'), trend: 'stable' as const }
      },
  ...
  private calculateStats(metrics: OptimizationMetric[]): Record<string, number> {
    if (metrics.length === 0) {
      return { averageExecutionTime: 0, averageEfficiency: 0, successRate: 0, totalOptimizations: 0 };
    }
    
    const successful = metrics.filter(m => m.success);
    return {
      averageExecutionTime: metrics.reduce((s, m) => s + m.executionTime, 0) / metrics.length,
      averageEfficiency: successful.reduce((s, m) => s + m.efficiency, 0) / (successful.length || 1),

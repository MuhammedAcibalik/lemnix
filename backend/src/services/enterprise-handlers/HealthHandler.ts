...
export class HealthHandler extends BaseHandler implements IHealthOperations {
  ...
  public async healthCheck(_req: Request, res: Response): Promise<void> {
    try {
      ...
      res.status(503).json({
        success: false,
        error: { code: 'HEALTH_CHECK_ERROR', message: error instanceof Error ? error.message : 'Unknown' },
        metadata: { timestamp: new Date().toISOString(), version: '5.0.0' }
      });
    }
  }
  ...
  public async getSystemHealth(_req: Request, res: Response): Promise<void> {
    try {
      ...
      const firstMetric = sysMetrics[0] as unknown as Record<string, unknown> | undefined;
      ...
      const app = firstMetric?.application as { averageResponseTime?: number } | undefined;
      ...
      this.sendSuccess(res, {
        ...
        services: {
          "Optimization Engine": { 
            status: health.status === 'healthy' ? 'operational' as const : 'degraded' as const, 
  ...
  public async getSystemHealthStatus(_req: Request, res: Response): Promise<void> {
    try {
      this.sendSuccess(res, {
        ...
        metrics: { cpuUsage: 0, memoryUsage: 0, diskUsage: 0, networkLatency: 0 },
        uptime: process.uptime(),
        version: '5.0.0'
      });
    } catch (error) {

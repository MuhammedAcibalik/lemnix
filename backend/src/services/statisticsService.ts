...
import { productionPlanService, ProductionPlanFilters } from './productionPlanService';
...
export class StatisticsService {
  ...
  async getStatistics(filters?: ProductionPlanFilters): Promise<ProductionPlanStatistics> {
    ...
    const departmentCompletedStats = await this.prisma.productionPlanItem.groupBy({
      by: ['bolum'],
      where: {
        ...this.buildItemWhereClause(filters),
        linkedCuttingListId: { not: null }
    ...
    const departmentDistribution = departmentStats.map(dept => {
      const completed = departmentCompletedStats.find(c => c.bolum === dept.bolum)?._count.id || 0;
      return {
   ...
   */
  private async getWeeklyTrends(filters?: ProductionPlanFilters): Promise<Array<{
    week: number;
    year: number;
    itemCount: number;
    completionRate: number;
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const completionRate = weekItems > 0 ? (weekCompleted / weekItems) * 100 : 0;
   ...
   */
  private async getTopBackorders(limit: number, filters?: ProductionPlanFilters): Promise<Array<{
    id: string;
    ad: string;
    siparis: string;
    planlananBitisTarihi: string;
    ...
    return backorderItems.slice(0, limit).map(item => {
      ...
      return {
        ...
        planlananBitisTarihi: typeof item.planlananBitisTarihi === 'string' 
   ...
   */
  private buildItemWhereClause(filters?: ProductionPlanFilters): Record<string, unknown> {
  ...
  private calculateDaysOverdue(planlananBitisTarihi: string | Date): number {
    const plannedDate = typeof planlananBitisTarihi === 'string' ? new Date(planlananBitisTarihi) : planlananBitisTarihi;
    const today = new Date();

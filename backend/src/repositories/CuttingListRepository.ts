...
import { CuttingList, CuttingListItem as PrismaCuttingListItem, Prisma } from '@prisma/client';
import { logger } from '../services/logger';
import { cacheService, CacheKeys, CacheTags } from '../services/cache/RedisCache';
...
export class CuttingListRepository {
   ...
   */
  public async create(data: Prisma.CuttingListCreateInput): Promise<CuttingList> {
    try {
  ...
  public async findById(id: string): Promise<CuttingListWithRelations | null> {
    try {
      ...
      try {
        cached = await cacheService.get<CuttingListWithRelations>(CacheKeys.cuttingList(id));
        if (cached) {
          return cached;
        }
      } catch (cacheError) {
    ...
    try {
      ...
      lists.forEach(list => {
        console.log(`🔍 List ${list.id} has ${list.items.length} items:`, 
          list.items.map(i => ({ id: i.id, profileType: i.profileType, quantity: i.quantity, length: i.length }))
        );
      });
   ...
   */
  public async findByWeekNumber(userId: string, weekNumber: number): Promise<CuttingList | null> {
    try {
      ...
      logger.error('Failed to find cutting list by week', { userId, weekNumber, error });
      throw error;
    }
  }
   ...
   */
  public async findAllByWeekNumber(weekNumber: number): Promise<ReadonlyArray<CuttingList>> {
    try {
      return await prisma.cuttingList.findMany({
        where: { weekNumber }
      });
    } catch (error) {
      logger.error('Failed to find cutting lists by week number', { weekNumber, error });
      throw error;
    }
  }
   ...
   * Update cutting list
   */
  public async update(id: string, data: Prisma.CuttingListUpdateInput): Promise<CuttingList> {
    try {
  ...
  public async findManyPaginated(options: {
    ...
    try {
      const { cursor, take = 50, userId, status, includeItems = false, includeStats = false } = options;
   ...
   */
  public async batchLoadByIds(ids: readonly string[]): Promise<(CuttingList | null)[]> {
    try {
      const lists = await prisma.cuttingList.findMany({
        where: { id: { in: [...ids] } },
        include: {
  ...
  ): Promise<{ id: string; productName: string; items: any[]; createdAt: string; updatedAt: string }> {
    try {
      ...
      console.log('🔍 Creating new section:', JSON.stringify(newSection, null, 2));
      ...
      try {
        existingSections = Array.isArray(cuttingList.sections) 
          ...
          : (cuttingList.sections ? JSON.parse(JSON.stringify(cuttingList.sections)) : []);
      } catch (parseError) {
        logger.warn('Failed to parse existing sections, starting fresh', { cuttingListId, parseError });
        existingSections = [];
      }

      // 🐛 DEBUG: Log existing sections
      console.log('🔍 Existing sections before update:', existingSections.length, existingSections.map(s => s.productName));

      const updatedSections = [...existingSections, newSection];
      
      // 🐛 DEBUG: Log what will be saved
      console.log('🔍 Updated sections to save:', updatedSections.length, updatedSections.map(s => s.productName));
      console.log('🔍 Full updated sections:', JSON.stringify(updatedSections.map(s => ({ id: s.id, productName: s.productName, itemsCount: s.items.length })), null, 2));
      ...
      const saved = await prisma.cuttingList.findUnique({ where: { id: cuttingListId } });
      console.log('🔍 Verified saved sections:', JSON.stringify(saved?.sections, null, 2));
      ...
      try {
        ...
        logger.warn('Cache invalidation failed (non-critical)', { cuttingListId, cacheError });
      }
  ...
  public async addItemToSection(
    ...
    try {
      ...
      try {
        ...
        logger.warn('Failed to parse existing sections', { cuttingListId, parseError });
        throw new Error('Invalid sections data');
      }

      // Find target section by sectionId - support both old and new ID formats
      const sectionIndex = existingSections.findIndex((s: CuttingListSection) => {
        // Direct match
        if (s.id === sectionId) {
          return true;
        }
        ...
        if (indexMatch) {
          ...
          return index < existingSections.length && existingSections[index] !== undefined;
        }
      ...
      console.log('🔍 Creating DB items for profiles:', itemData.profiles.length, 'profiles');
      const dbItems = [];
      for (const profile of itemData.profiles) {
        const profileData = {
          ...
          length: parseFloat(profile.measurement?.replace('mm', '') || '0') || 0, // Ölçü (mm'den sayıya çevir)
          cuttingPattern: null,
          materialNumber: null,
        ...
        console.log('🔍 Creating DB item:', JSON.stringify(profileData, null, 2));
      ...
      console.log('🔍 Verified items:', JSON.stringify(verifyItems.map(i => ({ 
        id: i.id, 
      ...
      logger.error('Failed to add item to section', { cuttingListId, sectionId, error });
      throw error;
    }
  }
  ...
  public async updateItemInSection(
    ...
    try {
      ...
      logger.info('🔍 Section matching debug', {
        ...
        sections: existingSections.map(s => ({ id: s.id, productName: s.productName }))
      });
      ...
      const sectionIndex = existingSections.findIndex((s: CuttingListSection) => {
        // Direct match
        if (s.id === sectionId) {
          logger.info('✅ Direct match found', { sectionId, matchedId: s.id });
          return true;
        }
        ...
        if (match) {
          const [, listId, index] = match;
          const expectedId = `section-${listId}-${index}`;
          logger.info('🔍 Checking new format', { sectionId, listId, index, expectedId, actualId: s.id });
          if (s.id === expectedId) {
            logger.info('✅ New format match found', { sectionId, expectedId, matchedId: s.id });
            return true;
          }
        }
        ...
        if (indexMatch) {
          const index = parseInt(indexMatch[1]);
          const isMatch = index < existingSections.length && existingSections[index] !== undefined;
          logger.info('🔍 Checking index fallback', { sectionId, index, isMatch, sectionExists: existingSections[index]?.id });
          if (isMatch) {
            logger.info('✅ Index fallback match found', { sectionId, index, matchedId: existingSections[index]?.id });
            return true;
          }
        }
      ...
      logger.info('🔍 Item matching debug', {
        ...
        items: targetSection.items?.map(item => ({ id: item.id, workOrderId: item.workOrderId })) || []
      });
      
      // Find target item - try by ID first, then by workOrderId if itemId starts with "workorder-"
      let itemIndex = targetSection.items?.findIndex((item: PrismaCuttingListItem) => item.id === itemId) ?? -1;
      ...
      if (itemIndex === -1 && itemId.startsWith('workorder-')) {
        ...
        logger.info('🔍 Trying to match by workOrderId', { itemId, workOrderId });
        itemIndex = targetSection.items?.findIndex((item: PrismaCuttingListItem) => item.workOrderId === workOrderId) ?? -1;
      }
      ...
      if (itemIndex === -1) {
        logger.error('❌ Item not found in section', {
          ...
          availableWorkOrderIds: targetSection.items?.map(item => item.workOrderId) || [],
          targetSectionId: targetSection.id
        });
      ...
      const now = new Date();
      const currentItem = targetSection.items[itemIndex] as PrismaCuttingListItem;
          ...
          for (const profile of itemData.profiles) {
            await prisma.cuttingListItem.create({
              data: {
                ...
                orderQuantity: itemData.orderQuantity || safeCurrentItem.orderQuantity,
                ...
                length: parseFloat(profile.measurement?.replace('mm', '') || '0') || 0,
                cuttingPattern: null,
                materialNumber: null,
                materialDescription: null,
                productionPlanItemId: null,
      ...
      logger.error('Failed to update item', { cuttingListId, sectionId, itemId, error });
      throw error;
    }
  }
  ...
  public async deleteItemFromSection(
    ...
    try {
      ...
      let filteredItems = (targetSection.items || []).filter((item: any) => item.id !== itemId);
      
      // If no items were filtered and itemId starts with "workorder-", try by workOrderId
      if (filteredItems.length === originalLength && itemId.startsWith('workorder-')) {
        const workOrderId = itemId.replace('workorder-', '');
        logger.info('🔍 Delete: Trying to match by workOrderId', { itemId, workOrderId });
        filteredItems = (targetSection.items || []).filter((item: any) => item.workOrderId !== workOrderId);
      }
      ...
      logger.error('Failed to delete item', { cuttingListId, sectionId, itemId, error });
      throw error;
    }
  }

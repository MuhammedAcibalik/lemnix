...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const report = await validationPolicyService.validateRecord(
      data,
      correlationId,
      recordId,
    );
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    const success = validationPolicyService.reviewQuarantineRecord(
      recordId,
      decision,
      reviewedBy,
      notes,
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const summary = {
      ...
      compliance: {
        ...
        dataFreshnessCompliance:
          Math.round(metrics.dataFreshnessCompliance * 10000) / 100, // percentage
      },
      ...
      quarantine: {
        ...
        reviewed: quarantineRecords.filter((r) => r.status === "REVIEWED")
          .length,
        resolved: quarantineRecords.filter((r) => r.status === "RESOLVED")
          .length,
      },
      ...
      health: {
        status:
          metrics.rejectionRate < 0.05 && metrics.dataFreshnessCompliance > 0.95
            ? "HEALTHY"
            : metrics.rejectionRate < 0.1 &&
                metrics.dataFreshnessCompliance > 0.9
              ? "WARNING"
              : "CRITICAL",
        issues: [] as string[],
      },
    ...
    if (metrics.rejectionRate >= 0.05) {
      healthIssues.push(
        `High rejection rate: ${summary.compliance.rejectionRate}%`,
      );
    }
    if (metrics.dataFreshnessCompliance < 0.95) {
      healthIssues.push(
        `Low data freshness compliance: ${summary.compliance.dataFreshnessCompliance}%`,
      );
    }
    if (metrics.averageProcessingTime > 5000) {
      healthIssues.push(
        `Slow processing time: ${summary.performance.averageProcessingTime}ms`,
      );
    }
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const recentRecords = quarantineRecords.filter(
      (r) => new Date(r.quarantinedAt).getTime() > oneDayAgo,
    );
      ...
      record.validationResults.forEach((result) => {
        if (!result.passed) {
          switch (result.severity) {
            case "CRITICAL":
              recentViolations.bySeverity.critical++;
              break;
            case "HIGH":
              recentViolations.bySeverity.high++;
              break;
            case "MEDIUM":
              recentViolations.bySeverity.medium++;
              break;
            case "LOW":
              recentViolations.bySeverity.low++;
              break;
          }
        }
      });
    ...
    res.json({
      ...
      data: {
        ...
        totalViolations:
          ...
          metrics.highViolations +
          metrics.mediumViolations +
          metrics.lowViolations,
      },
      timestamp: new Date().toISOString(),
    });
  }),
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const compliance = {
      ...
      status: {
        rejectionRate:
          metrics.rejectionRate <= 0.05 ? "COMPLIANT" : "NON_COMPLIANT",
        dataFreshness:
          metrics.dataFreshnessCompliance >= 0.95
            ? "COMPLIANT"
            : "NON_COMPLIANT",
        processingTime:
          metrics.averageProcessingTime <= 5000 ? "COMPLIANT" : "NON_COMPLIANT",
        quarantineRate:
          metrics.quarantineRate <= 0.1 ? "COMPLIANT" : "NON_COMPLIANT",
      },
    ...
    compliance.overallCompliance.score =
      totalCount > 0 ? (compliantCount / totalCount) * 100 : 0;
    compliance.overallCompliance.status =
      compliance.overallCompliance.score >= 75 ? "COMPLIANT" : "NON_COMPLIANT";

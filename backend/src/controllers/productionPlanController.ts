...
export class ProductionPlanController {
  ...
  async uploadProductionPlan(req: Request, res: Response): Promise<void> {
    try {
      ...
      logger.info('Upload request received', {
        ...
        fileName: req.file?.originalname ? req.file.originalname.substring(0, 20) + '...' : null, // ⚠️ Truncated filename
  ...
  async getProductionPlans(req: Request, res: Response): Promise<void> {
    try {
      ...
      const filters = {
        weekNumber: req.query.weekNumber ? Math.max(1, Math.min(53, Number(req.query.weekNumber))) : undefined,
        year: req.query.year ? Math.max(2020, Math.min(2030, Number(req.query.year))) : undefined,
        status: ['active', 'archived', 'draft'].includes(req.query.status as string) ? req.query.status as string : 'active',
        bolum: ['MONTAJ', 'HELEZON', 'KAYNAK', 'BOYA'].includes(req.query.bolum as string) ? req.query.bolum as string : undefined,
        oncelik: ['yuksek', 'orta', 'dusuk'].includes(req.query.oncelik as string) ? req.query.oncelik as string : undefined,
        page: Math.max(1, Number(req.query.page) || 1),
        limit: Math.max(1, Math.min(100, Number(req.query.limit) || 50))
      };

      const plans = await productionPlanService.getProductionPlans(filters, true);
  ...
  async getProductionPlanById(req: Request, res: Response): Promise<void> {
    try {
      ...
      logger.error('Get production plan by ID error', { error, planId: req.params.id });
      res.status(500).json({
        success: false,
        error: 'Sunucu hatası'
      });
  ...
  async deleteProductionPlan(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (result.success) {
        ...
        logger.error('Production plan delete failed', { planId: id, error: result.message });
        res.status(400).json({
          success: false,
      ...
      logger.error('Delete production plan error', { error, planId: req.params.id });
      res.status(500).json({
        success: false,
        error: 'Sunucu hatası'
      });
  ...
  async getProductionPlanMetrics(req: Request, res: Response): Promise<void> {
    try {
      ...
      const metrics = await productionPlanService.getProductionPlanMetrics(filters);
  ...
  async getProductionPlanByWeek(req: Request, res: Response): Promise<void> {
    try {
      ...
      const plan = await productionPlanService.getProductionPlanByWeek(weekNumber, year);
      ...
      logger.error('Get production plan by week error', { error, weekNumber: req.params.weekNumber, year: req.params.year });
      res.status(500).json({
        success: false,
        error: 'Sunucu hatası'
      });
   ...
   */
  async getProductionPlanItemsByWorkOrder(req: Request, res: Response): Promise<void> {
    try {
      ...
      const items = await productionPlanService.getProductionPlanItemsByWorkOrder(workOrderId);
      ...
      logger.error('Get production plan items by work order error', { error, workOrderId: req.params.workOrderId });
      res.status(500).json({
        success: false,
        error: 'Sunucu hatası'
      });
  ...
  async createCuttingListFromPlan(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Validation
      if (!productionPlanItems || !Array.isArray(productionPlanItems) || productionPlanItems.length === 0) {
        res.status(400).json({
          success: false,
      ...
      for (const item of productionPlanItems) {
        if (!item.planItemId || !item.profileType || !item.length || !item.quantity) {
          res.status(400).json({
            success: false,
            error: 'Tüm itemlar için plan ID, profil tipi, uzunluk ve miktar gerekli'
          });
          return;
        }
      ...
      const result = await productionPlanToCuttingListService.createCuttingListFromPlan({
        productionPlanItems,
        cuttingListMetadata,
        userId
      });
  ...
  async getLinkedPlanItems(req: Request, res: Response): Promise<void> {
    try {
      ...
      const planItems = await productionPlanToCuttingListService.getLinkedPlanItems(cuttingListId);
   ...
   */
  async unlinkPlanItemFromCuttingList(req: Request, res: Response): Promise<void> {
    try {
      ...
      await productionPlanToCuttingListService.unlinkPlanItemFromCuttingList(planItemId);
  ...
  async deleteAllProductionPlans(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (result.success) {
        ...
        logger.error('Delete all production plans failed', { error: result.message });
        res.status(400).json({
          success: false,
          error: result.message
        });
  ...
  async getBackorderItems(req: Request, res: Response): Promise<void> {
    try {
      ...
      const backorderItems = await productionPlanService.getBackorderItems(filters);

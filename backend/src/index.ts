...
async function bootstrap(): Promise<void> {
  ...
  httpServer.listen(env.PORT, async () => {
    try {
      ...
      const queryMonitoringStatus = getQueryMonitoringStatus();

      if (queryMonitoringStatus.enabled) {
        startQueryMonitoring();
      } else if (queryMonitoringStatus.reason) {
        logger.debug("Query monitoring not started", {
          reason: queryMonitoringStatus.reason,
        });
      }

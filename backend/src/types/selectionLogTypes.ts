...
export const DEFAULT_SELECTION_POLICIES: SelectionPolicy[] = [
  ...
  },
  {
    ...
    fallbackTriggers: [
      FallbackTrigger.TIMEOUT,
      FallbackTrigger.MEMORY_OVERFLOW,
    ],
    selectionCriteria: {
      qualityWeight: 0.5,
      speedWeight: 0.3,
...
export function getSelectionPolicy(
  ...
  const policy = DEFAULT_SELECTION_POLICIES.find(
    (policy) => policy.workloadClass === workloadClass,
  );
  return policy || DEFAULT_SELECTION_POLICIES[0];
}

export function calculateWorkloadComplexity(
  items: { profileType: string; stockLength: number }[],
): number {
...
export function estimateAlgorithmPerformance(
  ...
  const baseMetrics = {
    ...
    [AlgorithmType.BFD]: { duration: 150, quality: 0.9, memory: 15 },
    [AlgorithmType.NFD]: { duration: 80, quality: 0.8, memory: 8 },

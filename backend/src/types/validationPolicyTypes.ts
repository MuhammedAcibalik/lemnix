...
export const EDGE_CASE_RULES: EdgeCaseRule[] = [
  {
    ...
    fix: (data) => ({
      ...data,
      wastePercentage: Math.max(0, data.wastePercentage as number),
    }),
  ...
  },
  {
    ...
    condition: (data) => {
      ...
      return Boolean(
        (hasMm && hasInches) || (hasMm && hasCm) || (hasInches && hasCm),
      );
    },
    fix: (data) => {
    ...
    examples: [
      'length: "100mm" + width: "5in"',
      'length: "50cm" + height: "200mm"',
    ],
  },
  {
    name: "Excessive Kerf",
    ...
    condition: (data) =>
      Boolean((data.kerf as number) && (data.kerf as number) > 10), // > 10mm kerf is excessive
  ...
  },
  {
    ...
    condition: (data) => {
      return Boolean(
        ((data.length as number) && (data.length as number) <= 0) ||
          ...
          ((data.length as number) &&
            ...
            (data.length as number) < (data.width as number)) ||
          ((data.kerf as number) &&
            (data.length as number) &&
            (data.kerf as number) >= (data.length as number)),
      );
    },
    fix: (data) => {
      const fixed = { ...data };
      ...
      if (
        fixed.kerf &&
        fixed.length &&
        (fixed.kerf as number) >= (fixed.length as number)
      ) {
        fixed.kerf = Math.min(
          fixed.kerf as number,
          (fixed.length as number) * 0.1,
        ); // Max 10% of length
      }
      return fixed;
    },
  ...
  },
  {
    ...
    fix: (data) => ({
      ...data,
      quantity: Math.max(1, Math.round(data.quantity as number)),
    }),
...
export const VALIDATION_CATEGORIES = {
  ...
  [ValidationCategory.UNIT_CONSISTENCY]: ["Unit Confusion"],
  [ValidationCategory.BUSINESS_LOGIC]: ["Excessive Kerf"],
} as const;

// Helper functions
export function determineValidationAction(
  severity: ValidationSeverity,
  confidence: number,
): ValidationAction {
  switch (severity) {
...
export function isDataFresh(timestamp: string): boolean {
  return (
    ...
    VALIDATION_THRESHOLDS.DATA_FRESHNESS_SECONDS
  );
}

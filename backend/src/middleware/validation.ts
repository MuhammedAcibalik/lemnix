...
export const validateOptimizationParams = [
  body('algorithm')
    .isIn(['genetic', 'dynamic', 'greedy', 'simulated-annealing', 'branch-and-bound'])
  ...
  body('unit')
    .isIn(['mm', 'cm', 'm'])
    .withMessage('Geçersiz birim seçimi')
];
...
export const validateFileUpload = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (!allowedTypes.includes(file.mimetype)) {
    res.status(400).json({
      success: false,
      error: {
        message: 'Sadece Excel (.xlsx, .xls), CSV (.csv) ve JSON (.json) dosyaları kabul edilir',

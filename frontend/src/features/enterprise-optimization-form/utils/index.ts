...
import { LengthUnit, CuttingListItem, OptimizationConstraints, OptimizationParams } from '../types';
import { VALIDATION_LIMITS } from '../constants';

// Unit conversion utilities
export const convertUnit = (value: number, from: LengthUnit, to: LengthUnit): number => {
...
export const validateCuttingListItem = (item: CuttingListItem, index: number): Record<string, string> => {
...
export const validateConstraints = (constraints: OptimizationConstraints): Record<string, string> => {
  const errors: Record<string, string> = {};
  
  if (constraints.kerfWidth < 0) {
    errors.kerfWidth = 'Testere kalınlığı negatif olamaz';
  }
  if (constraints.maxWastePercentage < VALIDATION_LIMITS.MIN_WASTE_PERCENTAGE || 
...
export const transformToOptimizationItems = (
  ...
  return cuttingList.map(item => ({
    ...
    length: validateNumber(item.length, VALIDATION_LIMITS.MIN_LENGTH, VALIDATION_LIMITS.MAX_LENGTH),
    quantity: validateNumber(item.quantity, VALIDATION_LIMITS.MIN_QUANTITY, VALIDATION_LIMITS.MAX_QUANTITY),
...
export const transformToOptimizationFormData = (
  ...
  return {
    ...
    materialStockLengths: params.stockLengths.map((length: number) => ({
      ...
      stockLength: validateNumber(length, VALIDATION_LIMITS.MIN_STOCK_LENGTH, VALIDATION_LIMITS.MAX_STOCK_LENGTH),
      availability: 1000,
      costPerMm: 0.1,
      costPerStock: validateNumber(length, VALIDATION_LIMITS.MIN_STOCK_LENGTH, VALIDATION_LIMITS.MAX_STOCK_LENGTH) * 0.1,
      materialGrade: '6061-T6',
      weight: validateNumber(length, VALIDATION_LIMITS.MIN_STOCK_LENGTH, VALIDATION_LIMITS.MAX_STOCK_LENGTH) * 0.0027
    })),

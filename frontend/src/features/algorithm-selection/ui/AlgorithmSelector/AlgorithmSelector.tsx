...
export function AlgorithmSelector({ 
  ...
  const algorithms = useMemo(
    () => Object.values(ALGORITHM_CATALOG),
    []
  );
    ...
    <Card elevation={2}>
      <CardContent>
                          ...
                          <Stack spacing={1}>
                            ...
                            <Box>
                              <Box display="flex" justifyContent="space-between" alignItems="center" mb={0.5}>
                                ...
                                <Tooltip title={`Ölçeklenebilirlik: ${alg.scalability}/10`}>
                                  <Chip
                                    icon={<ScalabilityIcon sx={{ fontSize: 14 }} />}
                              ...
                              <LinearProgress 
                                ...
                                color={alg.scalability > 7 ? 'success' : alg.scalability > 5 ? 'primary' : 'warning'}
                            ...
                            {/* Recommended Range */}
                            <Typography variant="caption" color="text.secondary">
                              Önerilen: {alg.recommendedItemRange.min}-{alg.recommendedItemRange.max} parça
                            </Typography>
                          </Stack>
                          ...
                          {!isInRange && (
                            <Alert severity="warning" sx={{ mt: 1, py: 0 }}>
                              <Typography variant="caption">
                                Bu algoritma {itemCount} parça için optimal olmayabilir
                              </Typography>
                            </Alert>
                          )}
                        </Box>
        ...
        <Box mt={2} p={1.5} bgcolor="action.hover" borderRadius={1}>
          <Typography variant="caption" color="text.secondary">
            💡 <strong>İpucu:</strong> Genetic Algorithm çoklu hedef optimizasyonu ve 
            gelişmiş ayarları destekler. 10-200 parça arası için en iyi sonuçları verir.
          </Typography>
        </Box>
      </CardContent>
    </Card>

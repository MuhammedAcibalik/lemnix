 ...
 */
function calculateContainerMaxWidth(
  width: number,
  uiMode: UIMode,
): {
...
function calculateAdaptiveTokens(
  ...
  return {
    ...
    maxContentWidth:
      typeof containerConfig.maxWidth === "number"
        ? containerConfig.maxWidth
        : device.width,
    containerWidth:
      containerConfig.maxWidth === "none" ? "full" : "constrained",
    typography,
    spacing,
    components,
    borderRadius,
...
export function useAdaptiveVariant<T>(options: AdaptiveVariantOptions<T>): T {
...
export function useResponsiveValue<T>(options: ResponsiveValueOptions<T>): T {

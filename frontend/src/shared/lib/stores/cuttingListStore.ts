...
export const useCuttingListStore = create<CuttingListState & CuttingListActions>()(
  devtools(
    persist(
      (set, get) => ({
        ...initialState,
        ...
        // List management
        setCurrentList: (list: CuttingList | null) => set({ currentList: list }),
        ...
        updateList: (id: string, updates: Partial<CuttingList>) => set((state) => ({
        ...
        deleteList: (id: string) => set((state) => ({
        ...
        // Items management
        addItem: (item: CuttingListItem) => set((state) => {
        ...
        updateItem: (id: string, updates: Partial<CuttingListItem>) => set((state) => {
          if (!state.currentList) return state;
        ...
        removeItem: (id: string) => set((state) => {
          ...
          const itemToRemove = state.currentList.items.find(item => item.id === id);
          const updatedList = {
            ...state.currentList,
        ...
        selectMultipleItems: (ids: string[]) => set((state) => ({
        ...
        selectAll: () => set((state) => ({
          selectedItems: state.currentList?.items.map(item => item.id) || []
        })),
        ...
        setFilters: (filters: Partial<CuttingListState['filters']>) => set((state) => ({
        ...
        setSortOrder: (sortOrder: CuttingListState['sortOrder']) => set({ sortOrder }),
        toggleSort: (sortBy: CuttingListState['sortBy']) => set((state) => ({
          sortBy,
        ...
        bulkUpdateItems: (updates: Array<{ id: string; updates: Partial<CuttingListItem> }>) => {
          const { currentList } = get();
          if (!currentList) return;
        ...
        bulkDeleteItems: (ids: string[]) => {
          ...
          const items = currentList.items.filter(item => !ids.includes(item.id));

...
export const exportToPDF = async (
  ...
  try {
    ...
    doc.text("LEMNIX İSTATİSTİK RAPORU", pageWidth / 2, yPosition, {
      align: "center",
    });
    yPosition += 20;
    ...
    const workOrderData = [
      ...
      [
        ...
        `${data?.isEmirleri?.ortalamaIslemSuresi || 0} gün`,
      ],
    ];
    ...
    if (data?.profilAnalizi?.length > 0) {
      ...
      const profileData = data.profilAnalizi.map(
    ...
    if (data?.urunKategorileri?.length > 0) {
      ...
      const categoryData = data.urunKategorileri.map(
    ...
    if (data?.renkEbat?.renkler?.length > 0) {
      ...
      const colorData = data.renkEbat.renkler.map(
    ...
    if (data?.renkEbat?.ebatlar?.length > 0) {
      ...
      const sizeData = data.renkEbat.ebatlar.map(
...
const addProfessionalTitlePage = (doc: jsPDF, data: StatisticsData) => {
  ...
  doc.setTextColor(107, 114, 128);
  doc.text(
    "Bu rapor Lemnix Kesim Listesi Yönetim Sistemi tarafından otomatik olarak oluşturulmuştur.",
    pageWidth / 2,
    pageHeight - 30,
    { align: "center" },
  );
...
const addProfessionalTable = (
  ...
  doc.rect(
    x,
    y - data.length * rowHeight - headerHeight,
    width,
...
const createOverviewSection = (data: StatisticsData): PDFSection => {
  return {
    ...
    content: [
      {
        label: PDF_CONTENT.tr.labels.totalItems,
        value: `${data.kesimListesi.toplamUrun} ${PDF_CONTENT.tr.labels.count}`,
      },
      {
        label: PDF_CONTENT.tr.labels.totalCategories,
        value: `${data.kesimListesi.toplamKategori} ${PDF_CONTENT.tr.labels.count}`,
      },
      {
        label: PDF_CONTENT.tr.labels.averageQuantity,
        value: data.kesimListesi.ortalamaMiktar,
      },
      {
        label: PDF_CONTENT.tr.labels.totalWeight,
        value: `${data.kesimListesi.toplamAgirlik} kg`,
      },
      {
        label: PDF_CONTENT.tr.labels.completedOrders,
        value: `${data.isEmirleri.tamamlanan} ${PDF_CONTENT.tr.labels.count}`,
      },
      {
        label: PDF_CONTENT.tr.labels.pendingOrders,
        value: `${data.isEmirleri.bekleyen} ${PDF_CONTENT.tr.labels.count}`,
      },
      {
        label: PDF_CONTENT.tr.labels.totalOrders,
        value: `${data.isEmirleri.toplam} ${PDF_CONTENT.tr.labels.count}`,
      },
      {
        label: PDF_CONTENT.tr.labels.averageProcessingTime,
        value: `${data.isEmirleri.ortalamaIslemSuresi} gün`,
      },
    ],
  };
};

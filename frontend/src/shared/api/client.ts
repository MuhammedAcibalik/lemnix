...
import { apiCircuitBreaker, optimizationCircuitBreaker } from '../lib/circuitBreaker'; // ✅ P2-9
 ...
 */
function responseErrorInterceptor(error: AxiosError<ApiErrorResponse>): Promise<never> {
  ...
  if (error.response) {
    ...
    if (status === 401) {
      ...
      userMessage = data?.error?.message || 'Geçersiz istek. Lütfen verilerinizi kontrol edin.';
    }
...
function getCircuitBreaker(url: string) {
  if (url.includes('/enterprise/optimize') || url.includes('/enterprise/compare')) {
...
export const api = {
  get: <T>(url: string, config?: Parameters<typeof apiClient.get>[1]) => {
    ...
    return breaker.execute(() => handleApiResponse<T>(apiClient.get(url, config)));
  },
  
  post: <T>(url: string, data?: unknown, config?: Parameters<typeof apiClient.post>[2]) => {
    const breaker = getCircuitBreaker(url);
    return breaker.execute(() => handleApiResponse<T>(apiClient.post(url, data, config)));
  },
  
  put: <T>(url: string, data?: unknown, config?: Parameters<typeof apiClient.put>[2]) => {
    const breaker = getCircuitBreaker(url);
    return breaker.execute(() => handleApiResponse<T>(apiClient.put(url, data, config)));
  },
  
  patch: <T>(url: string, data?: unknown, config?: Parameters<typeof apiClient.patch>[2]) => {
    const breaker = getCircuitBreaker(url);
    return breaker.execute(() => handleApiResponse<T>(apiClient.patch(url, data, config)));
  },
  
  delete: <T>(url: string, config?: Parameters<typeof apiClient.delete>[1]) => {
    const breaker = getCircuitBreaker(url);
    return breaker.execute(() => handleApiResponse<T>(apiClient.delete(url, config)));
  },
};

...
export const getConfidenceLabel = (confidence: 'high' | 'medium' | 'low'): string => {
  switch (confidence) {
    case 'high':
      return 'Yüksek';
    case 'medium':
...
// Type guards
export const isProfileSuggestion = (value: unknown): value is ProfileSuggestion => {
  return (
    typeof value === 'object' &&
    value !== null &&
...
export const isSaveMappingRequest = (value: unknown): value is SaveMappingRequest => {
  return (
    typeof value === 'object' &&
    value !== null &&
    'malzemeNo' in value &&

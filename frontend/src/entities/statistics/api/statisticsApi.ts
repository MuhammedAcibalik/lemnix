...
export async function getStatisticsOverview(): Promise<StatisticsOverview> {
  try {
    ...
    if (err.response?.status === 401) {
      console.warn(
        "Statistics overview requires authentication - returning empty data",
      );
 ...
 */
export async function getAlgorithmPerformance(): Promise<
...
> {
  try {
    ...
    const validated = z
      .array(AlgorithmPerformanceSchema)
      .parse(response.data) as readonly AlgorithmPerformance[];
    return validated;
  } catch (error: unknown) {
    const err = error as { response?: { status?: number } };
    if (err.response?.status === 401) {
      console.warn(
        "Algorithm performance requires authentication - returning empty data",
      );
      return [];
    }
...
export async function getBatchStatistics(
  ...
  try {
    ...
    if (err.response?.status === 401) {
      console.warn(
        "Batch statistics requires authentication - returning empty data",
      );
    } else {
...
export async function getUsageAnalytics(
  ...
  try {
    const response = await api.get<UsageAnalytics>(STATS_ENDPOINTS.USAGE, {
      params: { days },
    });
    return response.data;
  } catch (error: unknown) {
 ...
 */
export async function getProfileUsageStats(
  limit: number = 50,
 ...
 */
export async function getCuttingListTrends(
  days: number = 30,
): Promise<CuttingListTrends> {
  try {
...
export async function getOptimizationAnalytics(): Promise<OptimizationAnalytics> {
  try {
    const response = await api.get<OptimizationAnalytics>(
      STATS_ENDPOINTS.OPTIMIZATION,
    );
    return response.data;
  } catch (error: unknown) {
    const err = error as { response?: { status?: number } };
...
export async function getWasteReductionTrends(): Promise<WasteReductionTrends> {
  try {
    const response = await api.get<WasteReductionTrends>(
      STATS_ENDPOINTS.WASTE_REDUCTION,
    );
    return response.data;
  } catch (error: unknown) {
    const err = error as { response?: { status?: number } };

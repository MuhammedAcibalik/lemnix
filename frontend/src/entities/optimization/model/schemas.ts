...
export const optimizationConstraintsSchema = z.object({
  ...
  maxWastePercentage: z
    .number()
    .min(0)
    .max(100, "Max waste percentage must be 0-100"),
  maxCutsPerStock: z
    .number()
    .int()
    .positive("Max cuts per stock must be positive integer"),
 ...
 */
export const optimizationParamsSchema = z
  .object({
    algorithm: algorithmTypeSchema,
    objectives: z
      .array(optimizationObjectiveSchema)
  ...
  .refine(
    (data) => {
      ...
      const totalWeight = data.objectives.reduce(
  ...
  .refine(
    (data) => {
      ...
      if (data.materialStockLengths && data.materialStockLengths.length > 0) {
        const itemProfiles = new Set(
          data.items.map((item) => item.profileType),
        );
        const stockProfiles = new Set(
          data.materialStockLengths.map((stock) => stock.profileType),
        );
...
export type PerformanceSettingsInput = z.infer<
  typeof performanceSettingsSchema
>;
export type OptimizationObjectiveInput = z.infer<
  typeof optimizationObjectiveSchema
>;
...
export const DEFAULT_OBJECTIVES = [
  {
    type: "maximize-efficiency" as const,
    weight: 0.5,

...
import { normalizeOptimizationResult, safeNormalizeOptimizationResult } from './dto';
...
export async function validateOptimizationRequest(
  ...
  if (!clientValidation.success) {
    return {
      valid: false,
      errors: clientValidation.error.errors.map(e => `${e.path.join('.')}: ${e.message}`),
    };
  }
  ...
  try {
    ...
    console.warn('Validation endpoint not available, skipping server-side validation');
    return { valid: true };
  }
}
...
export async function getOptimizationEstimate(
  ...
  try {
    ...
    const estimatedTime = request.params.algorithm === 'genetic' 
 ...
 */
export async function getAvailableAlgorithms(): Promise<ReadonlyArray<AlgorithmInfo>> {
  try {
    const response = await api.get<ReadonlyArray<AlgorithmInfo>>(ENDPOINTS.ALGORITHMS);
    return response.data;
  } catch (error) {
...
export async function getOptimizationResult(id: string): Promise<OptimizationResult> {
  ...
  if (!normalized) {
    throw new Error(`Failed to normalize optimization result ${id} from backend`);
  }

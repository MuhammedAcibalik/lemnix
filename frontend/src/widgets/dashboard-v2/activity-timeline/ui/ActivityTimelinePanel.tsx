...
import { Box, Typography, Stack, Chip, alpha, IconButton, Menu, MenuItem } from '@mui/material';
import {
...
import { useActivityTimeline, type ActivityEventType, type ActivityFilter } from '@/entities/dashboard';
  ...
  return (
    <CardV2 variant="glass" sx={{ p: ds.spacing['1'], height: '100%' }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: ds.spacing['2'] }}>
        ...
        >
          <MenuItem onClick={() => handleFilterChange(undefined)}>Tümü</MenuItem>
          <MenuItem onClick={() => handleFilterChange('optimization_started')}>Optimizasyonlar</MenuItem>
          <MenuItem onClick={() => handleFilterChange('cutting_list_created')}>Kesim Listeleri</MenuItem>
          <MenuItem onClick={() => handleFilterChange('export_generated')}>Dışa Aktarımlar</MenuItem>
        </Menu>
      </Box>
      ...
      {isLoading ? (
        ...
        <Box sx={{ textAlign: 'center', py: ds.spacing['6'] }}>
          <UserIcon sx={{ fontSize: 48, color: ds.colors.neutral[400], mb: ds.spacing['2'] }} />
          <Typography
            sx={{
              fontSize: '0.9375rem',
              fontWeight: 600,
        ...
        <Stack spacing={ds.spacing['2']} sx={{ maxHeight: 400, overflowY: 'auto', pr: ds.spacing['1'] }}>
 ...
 * Get event color
 */
function getEventColor(type: ActivityEventType, ds: ReturnType<typeof useDesignSystem>): string {

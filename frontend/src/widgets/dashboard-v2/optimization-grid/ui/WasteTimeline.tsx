...
export const WasteTimeline: React.FC<WasteTimelineProps> = ({
  ...
  const avgWastePercentage = useMemo(() => {
    if (data.length === 0) return 0;
    return data.reduce((sum, item) => sum + item.wastePercentage, 0) / data.length;
  }, [data]);
  ...
  return (
    <CardV2 variant="glass" sx={{ p: ds.spacing['1'], height: '100%' }}>
      {/* Header */}
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: ds.spacing['1'] }}>
        <Typography
          sx={{
            fontSize: '0.9375rem',
      ...
      >
        Toplam tasarruf: <Box component="span" sx={{ fontWeight: 600, color: ds.colors.success.main }}>{totalWasteSaved.toFixed(0)}m</Box>
      </Typography>

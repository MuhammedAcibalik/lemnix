 ...
 */
export const calculateProfileOptimization = (
  result: OptimizationResult,
): ProfileOptimizationResult[] => {
  const profileMap = new Map<
    string,
    { original: number; optimized: number; waste: number }
  >();

  result.cuts.forEach((cut) => {
    if (cut.profileType) {
      const existing = profileMap.get(cut.profileType) || {
        original: 0,
        optimized: 0,
        waste: 0,
      };
  ...
  return Array.from(profileMap.entries()).map(([profileType, data]) => ({
    ...
    efficiencyGain:
      data.original > 0
        ? ((data.optimized - data.original) / data.original) * 100
        : 0,

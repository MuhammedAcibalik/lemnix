...
export function GAMetricsCard({
  ...
  const actualGenerations =
    performanceMetrics?.actualGenerations ?? generations;
  const convergenceReason =
    performanceMetrics?.convergenceReason ?? "Bilinmiyor";
  const populationSize = performanceMetrics?.populationSize ?? 0;
  const effectiveComplexity =
    performanceMetrics?.effectiveComplexity ??
  ...
  const convergenceLabel = useMemo(() => {
    if (convergenceScore >= 90)
      return { label: "Mükemmel", color: "success" as const };
    if (convergenceScore >= 75)
      return { label: "İyi", color: "primary" as const };
    if (convergenceScore >= 50)
      ...
      <CardContent>
        ...
        <Alert severity="success" icon="⚡" sx={{ mb: 2 }}>
          <Typography variant="body2" fontWeight={600}>
            Çalışma Süresi:{" "}
            <strong>{(executionTimeMs / 1000).toFixed(2)} saniye</strong>
            {" | "}
            Jenerasyon:{" "}
            <strong>
              {actualGenerations}/{generations}
        ...
        <Stack spacing={2}>
          ...
          <Box display="grid" gridTemplateColumns="repeat(2, 1fr)" gap={2}>
            ...
            {/* Scalability Score */}
            {performanceMetrics?.scalability !== undefined &&
              performanceMetrics && (
                <Box>
                  <Box display="flex" alignItems="center" gap={0.5} mb={0.5}>
            ...
            {/* CPU Usage */}
            {performanceMetrics?.cpuUsage !== undefined &&
              performanceMetrics && (
                <Box>
                  <Box display="flex" alignItems="center" gap={0.5} mb={0.5}>
          ...
          <Box mt={1} p={1.5} bgcolor="action.hover" borderRadius={1}>
            <Typography variant="caption" color="text.secondary">
              💡 <strong>Detaylar:</strong> Erken durdurma ve adaptif mutasyon
              kullanıldı.
              {actualGenerations < generations &&

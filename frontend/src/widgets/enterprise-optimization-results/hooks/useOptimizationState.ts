...
export const useOptimizationState = (
  ...
  const [cuttingPlanModal, setCuttingPlanModal] =
    useState<CuttingPlanModalState>({
      open: false,
      stock: null,
    });

  const [kesimDetaylariModal, setKesimDetaylariModal] =
    useState<KesimDetaylariModalState>({
      open: false,
      workOrder: null,
    });
  ...
  const handleWorkOrderClick = useCallback(
    (workOrderId: string) => {
      const newExpanded =
        expandedWorkOrder === workOrderId ? null : workOrderId;
  ...
  const handleTextExplanation = useCallback(
    (
      cardId: string,
      group: { cuts: Cut[] },
      groupData: {
...
function generateCuttingPatternExplanation(
  group: { cuts: Cut[] },
  groupData: {
    totalPieces: number;
    barCount: number;
  ...
  group.cuts.forEach((cut: Cut, index: number) => {
    ...
    hasSegments &&
      (cut.segments as Segment[]).forEach(
  ...
  const efficiencyAnalysis = [
    {
      threshold: 90,
      message: "✅ Mükemmel verimlilik! Atık oranı çok düşük.\n",
      condition: (eff: number) => eff >= 90,
    },
    {
      threshold: 80,
      message: "✅ İyi verimlilik. Atık oranı kabul edilebilir seviyede.\n",
      condition: (eff: number) => eff >= 80,
    },
    {
      threshold: 70,
      message:
        "⚠️ Orta verimlilik. Atık oranı yüksek, optimizasyon önerilir.\n",
      condition: (eff: number) => eff >= 70,
    },
    {
      threshold: 0,
      message:
        "❌ Düşük verimlilik. Atık oranı çok yüksek, yeniden optimizasyon gerekli.\n",
      condition: () => true,
    },
  ].find(({ condition }) => condition(groupEfficiency));

  explanation +=
    efficiencyAnalysis?.message || "❌ Verimlilik analizi yapılamadı.\n";
  ...
  const recommendations = [
    {
      condition: avgRemaining > 200,
      message: `• Ortalama atık yüksek (${avgRemaining} mm). Parça boyutlarını gözden geçirin.\n`,
    },
    {
      condition: barCount > 10,
      message: `• Çok fazla profil kullanılıyor (${barCount} adet). Parça gruplandırmasını optimize edin.\n`,
    },
    {
      condition: groupEfficiency < 85,
      message: `• Farklı optimizasyon algoritması deneyin.\n`,
    },
  ];

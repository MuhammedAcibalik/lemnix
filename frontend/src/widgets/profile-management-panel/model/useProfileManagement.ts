...
export function useProfileDefinitions(activeOnly = true) {
  return useQuery({
    ...
    queryFn: () => ProfileManagementApi.getProfileDefinitions({ isActive: activeOnly }),
 ...
 */
export function useMappingsByWeek(weekNumber: number, year: number, enabled = true) {
  return useQuery({
    queryKey: ['profile-management', 'mappings', { weekNumber, year }] as const,
    queryFn: () => ProfileManagementApi.getWorkOrderProfileMappings({ weekNumber, year }),
    enabled,
...
export function useUploadProfileManagement() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['profile-management', 'definitions'] });
      queryClient.invalidateQueries({ queryKey: ['profile-management', 'mappings'] });
      queryClient.invalidateQueries({ queryKey: ['profile-management', 'statistics'] });
...
export function useUpdateMapping() {
  ...
  return useMutation({
    mutationFn: ({ id, profileCode }: { id: string; profileCode: string }) =>
      ProfileManagementApi.updateWorkOrderProfileMapping(id, { profileId: profileCode }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['profile-management', 'mappings'] });
    }
  });
...
export function useUpdateProfileDefinition() {
  ...
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: { profileCode: string; profileName: string } }) =>
...
export function useDeleteProfileDefinition() {
  ...
  return useMutation({
    mutationFn: (id: string) => ProfileManagementApi.deleteProfileDefinition(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['profile-management'] });
      queryClient.invalidateQueries({ queryKey: ['profile-management', 'definitions'] });
      queryClient.invalidateQueries({ queryKey: ['profile-management', 'statistics'] });

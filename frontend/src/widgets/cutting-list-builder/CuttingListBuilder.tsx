...
import { CuttingList, LoadingState, WorkOrderItem, ProfileCombination } from './types';
...
export const CuttingListBuilder: React.FC = () => {
  ...
  const {
    cuttingList, cuttingLists, title, selectedWeekNumber, productName,
    loadingState, error, success, showNewProductDialog, showNewItemDialog,
    showEditItemDialog, showCombinationDialog, currentSectionId, editingItem,
    expandedSections, newItemForm, availableSizes, profileCombinations,
    isLoadingSuggestions, currentWeekNumber, autoGeneratedTitle,
    cuttingListStats, isFormValid, resetNewItemForm, handleFormChange,
    addProfile, removeProfile, convertMeasurementToMM, handleProfileChange,
    handleEditItem, toggleSectionExpansion, copyItemToClipboard,
    setCuttingList, setCuttingLists, setTitle, setSelectedWeekNumber,
    setProductName, setLoadingState, setError, setSuccess,
    setShowNewProductDialog, setShowNewItemDialog, setShowEditItemDialog,
    setShowCombinationDialog, setCurrentSectionId, setEditingItem,
    setExpandedSections, setNewItemForm, setAvailableSizes,
  ...
  const handleApplySmartSuggestion = useCallback(async () => {
    if (!newItemForm.productName || !newItemForm.size || !newItemForm.orderQuantity) {
      setError('Ürün adı, ebat ve sipariş adedi gerekli');
      return;
    }
    ...
    try {
      ...
      console.log('📊 Smart Suggestion Response:', {
      ...
      if (result.profiles.length > 0) {
        // ✅ FIXED: Direct state update - no ID mismatch
        ...
        setSuccess(`✨ ${result.profiles.length} profil akıllı öneri ile eklendi! (Güven: %${result.totalConfidence})`);
      } else {
        setError('Bu ürün ve ebat için akıllı öneri bulunamadı');
      }
    } catch (error) {
      console.error('Smart suggestion failed:', error);
      setError('Akıllı öneri alınırken hata oluştu');
    } finally {
      setLoadingState(LoadingState.IDLE);
    }
  }, [newItemForm, applyAsync, setLoadingState, setNewItemForm, setError, setSuccess]);

  // ✅ DIRECT FETCH: Single state update, no intermediate layer
  const fetchCuttingLists = useCallback(async () => {
    console.log('[CuttingListBuilder] 🚀 Fetching cutting lists...');
    setLoadingState(LoadingState.LOADING);
    setError(null);
    ...
    try {
      ...
      if (result.success && result.data) {
        console.log('[CuttingListBuilder] ✅ Loaded:', result.data.length, 'lists');
        // DIRECT UPDATE - No intermediate state
        setCuttingLists(result.data);
        setLoadingState(LoadingState.SUCCESS);
      } else {
        console.warn('[CuttingListBuilder] ❌ API returned no data');
        setCuttingLists([]);
        setLoadingState(LoadingState.ERROR);
        setError('Kesim listeleri yüklenemedi');
      }
    } catch (error) {
  ...
  }, []); // ✅ Empty deps - setState functions are stable in React

  // ✅ FETCH ON MOUNT: Direct call, no cache
  useEffect(() => {
    fetchCuttingLists();
  }, [fetchCuttingLists]);

  // ✅ AUTO-DISMISS: Error messages auto-clear after 6 seconds
  useEffect(() => {
    if (error) {
  ...
  // ✅ AUTO-DISMISS: Success messages auto-clear after 4 seconds
  useEffect(() => {
    if (success) {
      const timer = setTimeout(() => {
        setSuccess(null);
  ...
  const {
    createCuttingList, addProductSection,
    addItemToSection, updateItemInSection, updateSection, deleteItem,
    deleteSection, exportToPDF, exportToExcel, handleError
  } = useCuttingListData({
    cuttingList, setCuttingList, setCuttingLists, title, selectedWeekNumber,
    productName, currentSectionId, newItemForm, editingItem, isFormValid,
    setLoadingState, setError, setSuccess, setShowNewProductDialog,
    setShowNewItemDialog, setShowEditItemDialog, setEditingItem, resetNewItemForm
  });

  const { getAvailableSizes, getProfileCombinations, applyProfileCombination } =
    useSmartSuggestions({
      setAvailableSizes, setProfileCombinations, setIsLoadingSuggestions,
      setNewItemForm, setShowCombinationDialog, setSuccess
    });

  // ✅ REMOVED: Old useEffect with loadCuttingListsFromBackend
  ...
  const handleAddItem = (sectionId: string) => {
    ...
    // ✅ CRITICAL FIX: Populate productName from section for smart suggestions
  ...
  const handleDeleteItem = (sectionId: string, itemId: string) => deleteItem(sectionId, itemId);
  ...
  const handleApplyCombination = (combination: ProfileCombination, orderQuantity: number) => {
    applyProfileCombination(combination, orderQuantity);
  };

  // Handle delete list from local state
  const handleDeleteList = (id: string) => {
    console.log('🗑️ Deleting list from local state:', id);
  ...
  return (
    ...
    >
      ...
      <NewItemDialog
        ...
        loadingState={{ isLoading: loadingState === LoadingState.LOADING, error: error || null }}
      ...
      <EditItemDialog
        ...
        onAddProfile={(itemId) => {
          ...
          if (editingItem) {
            ...
            setEditingItem({
              ...editingItem,
              profiles: [...(Array.isArray(editingItem.profiles) ? editingItem.profiles : []), newProfile]
            });
          }
        }}
      ...
      <CombinationDialog
        ...
        onSelectCombination={(combination) => handleApplyCombination(combination, 1)}

  ...
  return (
    ...
    >
      <Stack direction="row" alignItems="center" spacing={ds.spacing['1']}>
        {icon}
        <Box>
          <Typography sx={{ fontSize: '0.625rem', color: ds.colors.text.secondary, lineHeight: 1 }}>
            {label}
          </Typography>
          <Typography sx={{ fontSize: '0.75rem', fontWeight: 600, color: ds.colors.text.primary, lineHeight: 1.2 }}>
            {value}
          </Typography>
        </Box>
      </Stack>
      ...
      <Stack spacing={ds.spacing['2']}>
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between">
          <Stack direction="row" alignItems="center" spacing={ds.spacing['2']}>
            ...
            <Box>
              <Typography sx={{ fontSize: '0.875rem', fontWeight: 600, color: ds.colors.text.primary, lineHeight: 1, mb: 0.5 }}>
                {item.workOrderId}
              </Typography>
        ...
        <Grid container spacing={1}>
          <Grid item xs={6}>
            <DetailItem
              icon={<CalendarIcon sx={{ fontSize: ds.componentSizes.icon.small, color: ds.colors.primary.main }} />}
              label="Tarih"
              value={item.date || 'N/A'}
            />
          </Grid>
          ...
          <Grid item xs={6}>
            <DetailItem
              icon={<PaletteIcon sx={{ fontSize: ds.componentSizes.icon.small, color: ds.colors.error.main }} />}
              label="Renk"
              value={item.color}
            />
          </Grid>
          ...
          <Grid item xs={6}>
            <DetailItem
              icon={<StraightenIcon sx={{ fontSize: ds.componentSizes.icon.small, color: ds.colors.accent.main }} />}
              label="Ebat"
              value={item.size}
            />
          </Grid>
          ...
          <Grid item xs={6}>
            <DetailItem
              icon={<AssignmentIcon sx={{ fontSize: ds.componentSizes.icon.small, color: ds.colors.success.main }} />}
              label="Versiyon"
              value={item.version}
            />
          </Grid>
        ...
        {item.profiles && item.profiles.length > 0 && (
          <Box>
            <Typography sx={{ fontSize: '0.75rem', fontWeight: 600, mb: ds.spacing['1'], color: ds.colors.text.primary }}>

...
const CombinationCard: React.FC<{
  ...
  const profilesSummary = useMemo(() => {
    return combination.profiles
      .map(p => `${p.profile} ${p.measurement?.includes('mm') ? p.measurement : `${p.measurement}mm`}`)
      .join(', ');
      ...
      <CardContent sx={{ p: ds.spacing['2'] }}>
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between">
          <Stack direction="row" alignItems="center" spacing={ds.spacing['2']} sx={{ flex: 1 }}>
            ...
            <Box sx={{ flex: 1 }}>
              <Typography sx={{ fontSize: '1rem', fontWeight: 600, color: ds.colors.text.primary, mb: 0.5 }}>
...
export const CombinationDialog: React.FC<CombinationDialogProps> = ({
  ...
  const totalStats = useMemo(() => {
    const totalProfiles = combinations.reduce((sum, combo) => sum + combo.profiles.length, 0);
    const totalUsage = combinations.reduce((sum, combo) => sum + combo.usageCount, 0);
  ...
  return (
    ...
    >
      ...
      >
        <Stack direction="row" alignItems="center" justifyContent="space-between">
          <Box>
            <Stack direction="row" alignItems="center" spacing={ds.spacing['2']} sx={{ mb: ds.spacing['1'] }}>
              <SpeedIcon sx={{ 
                color: ds.colors.success.main,
            ...
            >
              {combinations.length} kombinasyon mevcut • Hızlı seçim için tıklayın
            </Typography>
          </Box>
          <IconButton
            onClick={onClose}
        ...
        <Stack spacing={ds.spacing['2']}>
          ...
          {isLoading ? (
            <Box sx={{ textAlign: 'center', py: ds.spacing['6'] }}>
              <SpeedIcon sx={{ fontSize: 48, color: ds.colors.primary.main, mb: ds.spacing['2'] }} />
              <Typography sx={{ fontSize: '1rem', fontWeight: 500, mb: ds.spacing['1'] }}>
            ...
            <Box sx={{ textAlign: 'center', py: ds.spacing['8'] }}>
              ...
              <Typography sx={{ 
                ...
                Profil kombinasyonları, iş emirlerinizi hızlıca oluşturmanıza yardımcı olur. 
                İlk kombinasyonunuz otomatik olarak kaydedilecek.
              </Typography>
            </Box>
          ) : (
            <Stack spacing={ds.spacing['2']}>

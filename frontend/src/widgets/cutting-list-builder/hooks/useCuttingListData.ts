...
import { addItemToSection as addItemToSectionAPI, createCuttingList as createCuttingListApi } from '@/entities/cutting-list/api/cuttingListApi';
...
export const useCuttingListData = ({
  ...
  const createCuttingList = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      console.log('✅ [createCuttingList] API call successful:', newCuttingList);
  ...
  }, [title, selectedWeekNumber, handleError, setCuttingList, loadCuttingListsFromBackend, setSuccess, setLoadingState]);

  const addProductSection = useCallback(async (productNameParam?: string): Promise<void> => {
    ...
    try {
      ...
      const { addProductSection } = await import('@/entities/cutting-list/api/cuttingListApi');
  ...
  }, [productName, cuttingList, handleError, setCuttingList, setShowNewProductDialog, setSuccess, setLoadingState]);
  ...
  const addItemToSection = useCallback(async (): Promise<void> => {
    ...
    try {
      const priorityMapped: 'low' | 'medium' | 'high' | 'urgent' = newItemForm.priority === '1' ? 'medium' : 'high';
      const itemData = {
        ...
        profiles: newItemForm.profiles.map(p => ({
          id: p.id,
          profile: p.profile,
          measurement: (p.measurement && !p.measurement.includes('mm'))
            ? `${p.measurement}mm`
            : p.measurement,
  ...
  }, [isFormValid, currentSectionId, cuttingList, newItemForm, handleError, setCuttingList, resetNewItemForm, setShowNewItemDialog, setSuccess, setLoadingState]);
  ...
  const updateItemInSection = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const itemData = {
        ...
        orderQuantity: typeof editingItem.orderQuantity === 'string' 
        ...
        profiles: (Array.isArray(editingItem.profiles) ? editingItem.profiles : []).map(p => ({
          id: p.id,
          profile: p.profile,
          measurement: (p.measurement && !p.measurement.includes('mm'))
            ? `${p.measurement}mm`
            : p.measurement,
          quantity: typeof p.quantity === 'string' ? parseInt(p.quantity) : p.quantity
        }))
      };
  ...
  }, [editingItem, currentSectionId, cuttingList, handleError, setCuttingList, resetNewItemForm, setEditingItem, setShowEditItemDialog, setSuccess, setLoadingState]);
  ...
  // Update Section
  const updateSection = useCallback(async (sectionId: string, updatedData: { productName: string }) => {
    if (!cuttingList) return;

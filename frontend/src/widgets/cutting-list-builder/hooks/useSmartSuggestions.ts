...
interface UseSmartSuggestionsProps {
  ...
  setNewItemForm: (form: WorkOrderForm | ((prev: WorkOrderForm) => WorkOrderForm)) => void;
...
export const useSmartSuggestions = ({
  ...
  const getAvailableSizes = useCallback(async (productName: string): Promise<void> => {
    ...
    try {
      ...
      if (error instanceof Error && (error as AxiosError).code !== 'ERR_CANCELED') {
  ...
  const applyProfileCombination = useCallback((
    ...
    setNewItemForm((prev: WorkOrderForm) => ({
      ...prev,

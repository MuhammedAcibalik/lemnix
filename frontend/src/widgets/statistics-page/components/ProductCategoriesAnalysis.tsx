...
export const ProductCategoriesAnalysis: React.FC<ProductCategoriesAnalysisProps> = ({ data }) => {
          ...
          <Card sx={{ 
            ...
            <CardContent>
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                ...
                <Box>
                  <Typography variant="h4" sx={{ fontWeight: 'bold' }}>
                    {(data?.categories || []).reduce((sum, cat) => sum + cat.totalQuantity, 0)}
                  </Typography>
                  <Typography variant="body2" sx={{ opacity: 0.9 }}>
                    Toplam Miktar
                  </Typography>
                    ...
                    <Box key={index} sx={{ 
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center' }}>
                        <Box sx={{ 
                          ...
                          height: 12,
                          borderRadius: '50%',
                          bgcolor: categoryDoughnutData.datasets[0].backgroundColor[index],
                          mr: 1
                        }} />
                    ...
                    <Box key={index} sx={{ 
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center' }}>
                        <Box sx={{ 
                          ...
                          height: 12,
                          borderRadius: '50%',
                          bgcolor: performanceDoughnutData.datasets[0].backgroundColor[index],
                          mr: 1
                        }} />
                        <Typography variant="body2" sx={{ 
                          fontWeight: 'bold',
                ...
                <Table>
                  <TableHead>
                    <TableRow sx={{ bgcolor: 'grey.50' }}>
                      ...
                      <TableCell align="center" sx={{ fontWeight: 'bold' }}>İş Emri Sayısı</TableCell>
                      <TableCell align="center" sx={{ fontWeight: 'bold' }}>Toplam Miktar</TableCell>
                      <TableCell align="center" sx={{ fontWeight: 'bold' }}>Kullanım Oranı</TableCell>
                      <TableCell align="center" sx={{ fontWeight: 'bold' }}>Performans</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {(data?.categoryPerformance || []).map((category, index) => (
                      <TableRow key={category.name} hover>
                        ...
                        <TableCell align="center">
                          <Chip 
                            label={parseFloat(category.percentage) > 30 ? 'Yüksek' : parseFloat(category.percentage) > 15 ? 'Orta' : 'Düşük'}
                            size="small"
                            color={parseFloat(category.percentage) > 30 ? 'success' : parseFloat(category.percentage) > 15 ? 'warning' : 'default'}
                            variant="outlined"
                          />
                        </TableCell>
                      </TableRow>
                ...
                {(data?.categories || []).map((category, index) => (
                  <Box key={category.name}>
                    ...
                    <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5, display: 'block' }}>
                      %{category.percentage} • {category.totalQuantity.toLocaleString()} toplam miktar
                    </Typography>
                    <Divider sx={{ mt: 2 }} />
                  </Box>
                ))}
                  ...
                  >
                    <CardContent sx={{ py: 2 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Box>
                          <Typography variant="h6" sx={{ fontWeight: 'bold', color: index === 0 ? 'primary.main' : 'text.primary' }}>
                            #{index + 1} {category.name}
                          </Typography>
                          <Typography variant="body2" color="text.secondary">
                            {category.itemCount} iş emri • {category.totalQuantity.toLocaleString()} miktar
                          </Typography>
                        </Box>
                        <Chip 

...
import { textContent, stylingConstants, accessibilityConstants } from '../constants';
...
export const ProductGroupSection: React.FC<ProductGroupSectionProps> = ({
  ...
  const totalItems = product.sections.reduce((sum, section) => 
  ...
  const selectedItems = product.sections.reduce((sum, section) => {
    ...
    return sum + section.items.filter(item => 
      workOrderSelection?.profiles[item.id]
    ).length;
  }, 0);

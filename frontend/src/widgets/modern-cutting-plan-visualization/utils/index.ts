 ...
 */
export const generateColorFromProfile: ColorGenerator = (profileType: string): string => {
  const colorMap = {
 ...
 */
export const getEfficiencyColor: EfficiencyColorGetter = (efficiency: number): string => {
  ...
  return efficiencyThresholds.find(({ threshold }) => efficiency >= threshold)?.color || '#f44336';
};
...
export const determinePriority: PriorityDeterminer = (length: number) => {
  ...
  return priorityThresholds.find(({ threshold }) => length > threshold)?.priority || 'low';
};
...
export const processOptimizationSegments = (
  optimizationResult: { cuts?: Array<{ segments?: Array<Record<string, unknown>> }> },
  stockLength: number
): ProcessedSegmentData[] => {
  if (!optimizationResult?.cuts) return [];

  return optimizationResult.cuts.flatMap((stock, stockIndex) => 
    stock.segments?.map(segment => ({
      ...
      workOrderId: segment.workOrderId ? String(segment.workOrderId) : undefined,
      workOrderItemId: segment.workOrderItemId ? String(segment.workOrderItemId) : undefined,
 ...
 */
export const groupSegmentsByProfileAndLength = (segments: ProcessedSegmentData[]): Record<string, GroupedSegmentData> => {
...
export const convertToCuttingStocks = (
  ...
  return Object.values(groupedSegments).map((group, index) => {
    ...
    const efficiency = totalStockLength > 0 ? (usedLength / totalStockLength) * 100 : 0;
...
export const calculateOverallStatistics = (
  ...
  const totalStocks = stocks.reduce((sum, stock) => sum + (stock.stockCount || 1), 0);
  const totalWaste = stocks.reduce((sum, stock) => sum + stock.wasteLength, 0);
  const totalUsedLength = stocks.reduce((sum, stock) => sum + (stock.usedLength || 0), 0);
  const totalStockLength = stocks.reduce((sum, stock) => sum + (stock.totalStockLength || 0), 0);
  
  const overallEfficiency = totalStockLength > 0 ? (totalUsedLength / totalStockLength) * 100 : 0;
  
  const weightedEfficiency = stocks.length > 0 
    ? stocks.reduce((sum, stock) => sum + (stock.efficiency * (stock.stockCount || 1)), 0) / totalStocks
    : 0;
  
  const totalPieces = stocks.reduce((sum, stock) => sum + (stock.cuts[0]?.quantity || 0), 0);
  const wastePercentage = totalStockLength > 0 ? (totalWaste / totalStockLength) * 100 : 0;
  
  // Cost calculation (example price: 45.50 TL/m)
  const materialCost = (totalStockLength / 1000) * 45.50;

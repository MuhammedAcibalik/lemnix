...
import { useActiveProfiles, ProfileManagementApi } from '@/entities/profile-management/api/profileManagementApi';
 ...
 */
export const DetailedSelectionDialog: React.FC<DetailedSelectionDialogProps> = ({
  open,
  onClose,
  cuttingList,
  onConfirm,
  ...
  const [bulkSelectMode, setBulkSelectMode] = useState<'all' | 'none' | null>(null);
  ...
  const { data: activeProfiles, isLoading: profilesLoading, refetch: refetchActiveProfiles } = useActiveProfiles();
  ...
  const { data: mappings, isLoading: mappingsLoading, refetch: refetchMappings } = useQuery({
    ...
    queryFn: () => ProfileManagementApi.getWorkOrderProfileMappings({ weekNumber: week, year }),
    enabled: open,
  ...
  useEffect(() => {
    if (open) {
      console.log('[DetailedSelectionDialog] 🔄 Dialog opened, refreshing profile data...');
      // Invalidate and refetch
      queryClient.invalidateQueries({ queryKey: ['profile-management', 'active'] });
      queryClient.invalidateQueries({ queryKey: ['workOrderProfileMappings', week, year] });
  ...
  useEffect(() => {
    ...
    if (open && cuttingList?.sections) {
      console.log('[DetailedSelectionDialog] 🔄 Initializing/Resetting selection state');
      const initialProducts: Record<string, ProductSelection> = {};
      
      
      cuttingList.sections.forEach((section: CuttingListSection) => {
        const workOrders: WorkOrderSelection[] = section.items.map((item: CuttingListItem) => {
      ...
      console.log('[DetailedSelectionDialog] ✅ Selection state initialized with', Object.keys(initialProducts).length, 'products');
    }
  }, [cuttingList, open]); // ✅ FIX: Added `open` dependency

  // Calculate statistics
  const statistics = useMemo(() => {
    console.log('[DetailedSelectionDialog] 🔵 useMemo recalculating statistics...');
    let totalProfiles = 0;
    let totalQuantity = 0;
    ...
    const result = {
      ...
      selectedProducts,
      totalWorkOrders: Object.values(selectionState.products).reduce((sum, p) => sum + p.workOrders.length, 0),
      selectedWorkOrders,
      totalProfiles,
      selectedProfiles,
      totalQuantity,
  ...
  const handleProductToggle = (productId: string) => {
    console.log('[DetailedSelectionDialog] 🔵 handleProductToggle called:', { productId });
    
    setSelectionState(prev => {
      const product = prev.products[productId];
      if (!product) {
        console.error('[DetailedSelectionDialog] ❌ Product not found:', productId);
        return prev;
      }
      ...
      const totalProfiles = updatedWorkOrders.reduce((sum, wo) => sum + wo.profiles.length, 0);
      const selectedProfiles = updatedWorkOrders.reduce((sum, wo) => 
        sum + wo.profiles.filter(p => p.selected).length, 0
      );
  ...
  const handleWorkOrderToggle = (productId: string, workOrderId: string) => {
    console.log('[DetailedSelectionDialog] 🔵 handleWorkOrderToggle called:', { productId, workOrderId });
    ...
    setSelectionState(prev => {
      ...
      const workOrderIndex = product.workOrders.findIndex(w => w.workOrderId === workOrderId);
      if (workOrderIndex === -1) {
        console.error('[DetailedSelectionDialog] ❌ WorkOrder not found:', workOrderId);
        return prev;
      }
      ...
      console.log('[DetailedSelectionDialog] 🔵 WorkOrder current state:', {
        ...
        newSelected,
        profileCount: Array.isArray(workOrder.profiles) ? workOrder.profiles.length : 0
      });
      ...
      console.log('[DetailedSelectionDialog] ✅ WorkOrder toggled, new state:', {
        workOrderId,
        newSelected,
  ...
  const handleProfileToggle = (productId: string, workOrderId: string, profileId: string) => {
    console.log('[DetailedSelectionDialog] 🔵 handleProfileToggle called:', { productId, workOrderId, profileId });
    
    setSelectionState(prev => {
      console.log('[DetailedSelectionDialog] 🔍 BEFORE toggle - prev.products:', Object.keys(prev.products));
      
      const product = prev.products[productId];
      if (!product) {
        console.error('[DetailedSelectionDialog] ❌ Product not found:', productId, 'Available products:', Object.keys(prev.products));
        return prev;
      }
      
      console.log('[DetailedSelectionDialog] ✅ Product found:', product.productId, 'workOrders:', product.workOrders.length);
      
      const workOrderIndex = product.workOrders.findIndex(w => w.workOrderId === workOrderId);
      if (workOrderIndex === -1) {
        console.error('[DetailedSelectionDialog] ❌ WorkOrder not found:', workOrderId, 'Available workOrders:', product.workOrders.map(w => w.workOrderId));
        return prev;
      }
      
      const workOrder = product.workOrders[workOrderIndex];
      console.log('[DetailedSelectionDialog] ✅ WorkOrder found:', workOrder.workOrderId, 'profiles:', Array.isArray(workOrder.profiles) ? workOrder.profiles.length : 0);
      
      if (!Array.isArray(workOrder.profiles)) {
        console.error('[DetailedSelectionDialog] ❌ WorkOrder has no profiles array:', workOrder.workOrderId);
        return prev;
      }
      
      const profileIndex = workOrder.profiles.findIndex(p => p.profileId === profileId);
      if (profileIndex === -1) {
        console.error('[DetailedSelectionDialog] ❌ Profile not found:', profileId, 'Available profiles:', workOrder.profiles.map(p => p.profileId));
        return prev;
      }
      ...
      console.log('[DetailedSelectionDialog] 🔵 AFTER toggle - newState products:', Object.keys(newState.products));
      console.log('[DetailedSelectionDialog] 🔵 Updated profile selected:', newSelected);
  ...
  const handleConfirm = () => {
    ...
    console.log('[DetailedSelectionDialog] 🔍 handleConfirm called, selectionState:', {
    ...
    Object.values(selectionState.products).forEach(product => {
      console.log('[DetailedSelectionDialog] 🔍 Processing product:', product.productId);
      
      product.workOrders.forEach(workOrder => {
        if (workOrder.selected) {
          console.log('[DetailedSelectionDialog] ✅ WorkOrder selected:', workOrder.workOrderId);
          ...
          if (Array.isArray(workOrder.profiles)) {
            ...
            console.warn('[DetailedSelectionDialog] ⚠️ WorkOrder has no profiles array:', workOrder.workOrderId);
          }
        }
      });
    });

    console.log('[DetailedSelectionDialog] 🔍 Total selected items:', selectedItems.length);
    onConfirm(selectedItems, selectedProfiles);
  };

  // Profile selection handler
  const handleProfileSelect = (profileType: string, profile: ProfileSelectionMap[string]) => {
  ...
  return (
    <Dialog
      ...
      PaperProps={{
        sx: {
          ...
          boxShadow: '0 25px 80px rgba(15, 23, 42, 0.2), 0 10px 32px rgba(0, 0, 0, 0.12)',
          maxHeight: '90vh',
          minHeight: '80vh',
        }
      }}
    >
      ...
      <DialogContent sx={{ p: 0 }}>
        {/* Statistics Panel */}
        {showStatistics && (
          <Box sx={{ p: 3, background: 'linear-gradient(135deg, rgba(37, 99, 235, 0.05) 0%, rgba(59, 130, 246, 0.08) 100%)' }}>
            <Grid container spacing={3}>
              <Grid item xs={12} sm={6} md={3}>
                <Paper sx={{ 
                  ...
                  <Typography variant="h4" sx={{ fontWeight: 800, color: '#1d4ed8', mb: 0.5 }}>
              ...
              <Grid item xs={12} sm={6} md={3}>
                <Paper sx={{ 
                  ...
                  <Typography variant="h4" sx={{ fontWeight: 800, color: '#0ea5e9', mb: 0.5 }}>
              ...
              <Grid item xs={12} sm={6} md={3}>
                <Paper sx={{ 
                  p: 2, 
                  textAlign: 'center',
                  background: 'linear-gradient(135deg, rgba(245, 158, 11, 0.1) 0%, rgba(251, 146, 60, 0.15) 100%)',
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, ml: 1 }}>
                      ...
                      <Box>
                        ...
                        <Typography variant="body2" sx={{ color: '#64748b' }}>
                          {product.workOrders.length} iş emri, {product.workOrders.reduce((sum, w) => sum + w.profiles.length, 0)} profil
                        </Typography>
                      </Box>
                    </Box>
                  }
                ...
                <Box sx={{ p: 2 }}>
                  {product.workOrders.map((workOrder) => (
                    <Box key={workOrder.workOrderId} sx={{ mb: 2 }}>
                      <Card sx={{ 
                        borderRadius: '10px',
                        border: workOrder.selected ? '2px solid #10b981' : '1px solid rgba(148, 163, 184, 0.2)',
                        ...
                        <CardContent sx={{ p: 2 }}>
                          <FormControlLabel
                            control={
                              <Checkbox
                                ...
                                onChange={() => handleWorkOrderToggle(product.productId, workOrder.workOrderId)}
                                icon={<CheckBoxOutlineBlankIcon />}
                                checkedIcon={<CheckBoxIcon />}
                                sx={{
                                  color: '#10b981',
                            ...
                            label={
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, ml: 1 }}>
                                <AssignmentIcon sx={{ color: '#10b981', fontSize: 20 }} />
                                <Box sx={{ flex: 1 }}>
                                  ...
                                  <Stack direction="row" spacing={2} sx={{ mt: 0.5 }}>
                                    <Chip
                                      ...
                                      size="small"
                                      sx={{ background: 'rgba(59, 130, 246, 0.1)', color: '#2563eb' }}
                                    />
                                    <Chip
                                    ...
                                    <Chip
                                      ...
                                      size="small"
                                      sx={{ background: 'rgba(245, 158, 11, 0.1)', color: '#f59e0b' }}
                                    />
                                  </Stack>
                                </Box>
                                <IconButton
                                  onClick={() => handleWorkOrderExpand(product.productId, workOrder.workOrderId)}
                                  size="small"
                                >
                                  {workOrder.expanded ? <ExpandLessIcon /> : <ExpandMoreIcon />}
                                </IconButton>
                              </Box>
                            }
                          ...
                          <Collapse in={workOrder.expanded}>
                            <Box sx={{ mt: 2, pl: 4 }}>
                              ...
                              <Stack spacing={2}>
                                {Array.isArray(workOrder.profiles) ? workOrder.profiles.map((profile) => (
                                  <Box key={profile.profileId}>
                                    <Card sx={{
                                      border: profile.selected ? '2px solid #2563eb' : '1px solid rgba(148, 163, 184, 0.2)',
                                      ...
                                      <CardContent sx={{ p: 2 }}>
                                        <Box sx={{ display: 'flex', alignItems: 'start', gap: 2 }}>
                                          <Checkbox
                                            ...
                                            onChange={() => handleProfileToggle(product.productId, workOrder.workOrderId, profile.profileId)}
                                            sx={{
                                              color: '#2563eb',
                                              '&.Mui-checked': {
                                                color: '#1d4ed8',
                                          ...
                                          <Box sx={{ flex: 1 }}>
                                            ...
                                            <FormControl fullWidth size="small" disabled={profilesLoading || mappingsLoading}>
                                              <InputLabel>Profil Tanımı Seç</InputLabel>
                                              <Select
                                                value={selectedProfiles[profile.profile]?.profileId || ''}
                                                onChange={(e) => {
                                                  const selectedProfile = activeProfiles?.find(p => p.profileId === e.target.value);
                                                  handleProfileSelect(profile.profile, selectedProfile || null);
                                                }}
                                              ...
                                              >
                                                ...
                                                {(() => {
                                                  const allowedProfileIds = profileTypeToProfileIds.get(profile.profile) || [];
                                                  return activeProfiles
                                                    ?.filter(pro => allowedProfileIds.includes(pro.profileId))
                                                    .map(pro => (
                                                      <MenuItem key={pro.profileId} value={pro.profileId}>
                                                        <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                                                          ...
                                                          <Typography variant="caption" color="text.secondary">
                                                            {pro.profileName} - {pro.stockLengths.length} stok boyu
                                                          </Typography>
                                                        </Box>
                                                      </MenuItem>
                                                    ));
      ...
      <DialogActions sx={{ 
        ...
        >
          {loading ? 'İşleniyor...' : `${statistics.selectedProfiles} Profil Seç`}
        </Button>
      </DialogActions>
    </Dialog>
  );

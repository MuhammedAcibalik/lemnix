 ...
 */
export const WasteDistributionChart: React.FC<WasteDistributionChartProps> = ({
  wasteDistribution,
}) => {
  const ds = useDesignSystem();

  const wasteCategoryData = useMemo(
    () => aggregateWasteByCategory(wasteDistribution),
    [wasteDistribution],
  );
        ...
        tooltip: {
          ...
          callbacks: {
            label: (context) => {
              ...
              const percentage =
                wasteCategoryData[context.dataIndex]?.percentage.toFixed(1) ||
                "0";

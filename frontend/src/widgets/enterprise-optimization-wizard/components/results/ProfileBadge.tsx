...
import { AccountTree as ProfileIcon, Check as CheckIcon } from '@mui/icons-material';
...
export const ProfileBadge: React.FC<ProfileBadgeProps> = ({
  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: ds.spacing['2'] }}>
        <Box
          sx={{
            width: 40,
            height: 40,
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: ds.spacing['1'], mb: 0.5 }}>
            ...
            <Tooltip title={source === 'mapping' ? 'Sipariş eşleştirmesinden' : 'Varsayılan profil'}>
              <Chip
                icon={source === 'mapping' ? <CheckIcon sx={{ fontSize: 10 }} /> : undefined}

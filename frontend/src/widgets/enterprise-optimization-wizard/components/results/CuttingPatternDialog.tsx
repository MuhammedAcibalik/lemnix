      ...
      <DialogContent sx={{ p: ds.spacing['4'] }}>
        ...
        <Box sx={{ mb: ds.spacing['3'] }}>
          ...
          <Stack direction="row" spacing={ds.spacing['3']}>
            <Box>
              ...
              <Typography sx={{ fontSize: '1.25rem', fontWeight: 700, color: ds.colors.primary.main }}>
            ...
            <Box>
              ...
              <Typography sx={{ fontSize: '1.25rem', fontWeight: 700, color: ds.colors.info.main }}>
            ...
            <Box>
              ...
              <Typography sx={{ fontSize: '1.25rem', fontWeight: 700, color: ds.colors.success.main }}>
                {plan.efficiency}%
              </Typography>
            </Box>
          </Stack>
        ...
        {plan.workOrderCount && plan.workOrderCount > 1 && (
          <Box sx={{
            ...
            <Typography sx={{ 
              ...
              Bu optimizasyon {plan.workOrderCount} farklı iş emrini birleştirerek gerçekleştirildi.
              Her kesim planında hangi iş emrinden kaç parça kesildiği detaylı olarak gösterilmektedir.
            </Typography>
          </Box>
        )}
        ...
        <Box sx={{
          ...
          {(() => {
            ...
            const segmentTotals = plan.cuts.reduce((acc, cutDetail) => {
              cutDetail.segments.forEach(segment => {
                const existing = acc.find(item => item.length === segment.length);
                if (existing) {
                ...
                {segmentTotals.map((item, idx) => (
                  ...
                  >
                    ...
                    <Typography sx={{
                      ...
                      ({plan.stockCount} stok boyundan, ort. {(item.totalCount / plan.stockCount).toFixed(1)} parça/stok)
                    </Typography>
                  </Box>
                ))}
              </Stack>
        ...
        <Stack spacing={ds.spacing['2']}>
          {plan.cuts.map((cutDetail, idx) => {
            ...
            const groupedSegments = cutDetail.segments.reduce((acc, segment) => {
              const existing = acc.find(item => item.length === segment.length);
              if (existing) {
            ...
            return (
              ...
              >
                ...
                <Stack direction="row" spacing={ds.spacing['2']} alignItems="center">
                  ...
                  <Typography sx={{ fontSize: '0.75rem', color: ds.colors.success.main, fontWeight: 600 }}>

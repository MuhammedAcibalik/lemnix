 ...
 */
export const ResultsHeroMetrics: React.FC<ResultsComponentProps> = ({
  result,
  loading = false,
}) => {
  ...
  const efficiency =
    result.efficiency ||
    (result as { totalEfficiency?: number }).totalEfficiency ||
    0;
  const wastePercentage = result.wastePercentage || 0;
  const wasteDistribution = (
    result as { wasteDistribution?: { reclaimable?: number } }
  ).wasteDistribution;
  const stockCount =
    (result as { stockCount?: number }).stockCount ||
    (result as { totalStocks?: number }).totalStocks ||
    result.cuts?.length ||
    0;
  const costBreakdown = (
    result as { costBreakdown?: { materialCost?: number } }
  ).costBreakdown;
  ...
  const avgSegmentsPerStock =
    cuts.length > 0
      ? cuts.reduce(
          (sum: number, c) =>
            sum + ((c as { segments?: unknown[] }).segments?.length || 1),
          0,
        ) / cuts.length
      : 0;
  ...
  const metrics = [
    ...
    },
    {
      ...
      icon: WasteIcon,
      color:
        wastePercentage < 5 ? ds.colors.success.main : ds.colors.warning.main,
      bgColor: alpha(
        wastePercentage < 5 ? ds.colors.success.main : ds.colors.warning.main,
        0.1,
      ),

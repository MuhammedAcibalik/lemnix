  ...
  if (recommendations.length === 0) {
    return (
      <CardV2 variant="glass" sx={{ p: ds.spacing["4"], height: "100%" }}>
        ...
        >
          <TrendingUpIcon
            sx={{ fontSize: 18, color: ds.colors.success.main }}
          />
          <Typography
            sx={{
              fontSize: "0.9375rem",
      ...
      >
        {recommendations.map((rec, index) => {
          ...
          let IconElement: React.ReactNode = (
            <LightbulbIcon sx={{ fontSize: 16, color: priorityColor }} />
          );
          if (rec.type === "performance")
            IconElement = (
              <PerformanceIcon sx={{ fontSize: 16, color: priorityColor }} />
            );
          else if (rec.type === "cost")
            IconElement = (
              <CostIcon sx={{ fontSize: 16, color: priorityColor }} />
            );
          else if (rec.type === "quality")
            IconElement = (
              <QualityIcon sx={{ fontSize: 16, color: priorityColor }} />
            );
          else if (rec.type === "waste")
            IconElement = (
              <WasteIcon sx={{ fontSize: 16, color: priorityColor }} />
            );

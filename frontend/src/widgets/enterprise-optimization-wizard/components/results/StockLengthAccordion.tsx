...
export const StockLengthAccordion: React.FC<StockLengthAccordionProps> = ({
  ...
  const mappedPlans = useMemo(() => {
    return cuts.map((cut, idx) => {
      ...
      const displayWorkOrder = workOrderIds.length > 0 
        ? workOrderIds[0] || 'N/A'
        : 'N/A';
      ...
      return {
        ...
        efficiency: cut.stockLength > 0 ? ((cut.stockLength - cut.remainingLength) / cut.stockLength) * 100 : 0,
  ...
  return (
    ...
    >
      ...
      >
        <Stack direction="row" spacing={ds.spacing['3']} alignItems="center" width="100%">
          {/* Stock Length Label */}
          <Box sx={{ display: 'flex', alignItems: 'center', gap: ds.spacing['2'] }}>
            <InventoryIcon sx={{ color: ds.colors.primary.main, fontSize: 20 }} />
            <Typography variant="h6" sx={{ fontWeight: 600, color: ds.colors.text.primary }}>
          ...
          <Chip 
            ...
            sx={{ 
              fontWeight: 500,
              backgroundColor: getWasteColor(wastePercentage) === 'success' 
                ? ds.colors.success[300]
                : getWasteColor(wastePercentage) === 'warning'
                ? ds.colors.warning[300]
                : ds.colors.error[300],
              color: getWasteColor(wastePercentage) === 'success' 
      ...
      <AccordionDetails sx={{ p: 0, backgroundColor: ds.colors.background.default }}>
        {/* Detailed cutting plan table */}
        <Box sx={{ p: ds.spacing['4'] }}>
          <Typography variant="subtitle1" sx={{ mb: ds.spacing['3'], fontWeight: 600 }}>

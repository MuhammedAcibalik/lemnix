...
import type { AlgorithmMode, ParetoOptimizationResult } from '@/entities/algorithm';
...
export const EnterpriseOptimizationWizard: React.FC = () => {
  ...
  const [selectedCuttingList, setSelectedCuttingList] = useState<CuttingListData | null>(null);
  ...
  const loadCuttingLists = useCallback(async () => {
    ...
    try {
      const response = await apiClient.get<{ success: boolean; data?: CuttingListData[] }>('/cutting-list');
   ...
   */
  const handleCuttingListSelect = useCallback((list: CuttingListData) => {
    console.log('[EnterpriseWizard v2] 🎯 Cutting list selected:', list.id);
    setSelectedCuttingList(list);
    
    // Open detailed selection dialog
    console.log('[EnterpriseWizard v2] 🔓 Opening detailed selection dialog...');
   ...
   */
  const handleParamsChange = useCallback((newParams: OptimizationParams) => {
    setParams(newParams);
  ...
  const handleStartOptimization = useCallback(async () => {
    ...
    if (!selectedCuttingList.sections || selectedCuttingList.sections.length === 0) {
      setOptimizationResult({
        success: false,
        message: 'Seçilen kesim listesinde ürün grubu bulunmuyor. Lütfen Kesim Listesi Oluşturucu sayfasından ürün ekleyin.',
        error: 'NO_SECTIONS'
      });
      return;
    }
    ...
    const items = params.items && params.items.length > 0
      ? (() => {
          console.log('[EnterpriseWizard v2] ✅ Using params.items for optimization');
          console.log('[EnterpriseWizard v2] 🔍 Sample params.items:', params.items.slice(0, 2));
          return params.items.map(item => {
            ...
            const length = typeof item.measurement === 'string' 
      ...
      : (() => {
          console.log('[EnterpriseWizard v2] ⚠️ Falling back to all items from cutting list');
          return selectedCuttingList.sections?.flatMap(section => {
            return section.items?.flatMap(item => {
              ...
              return item.profiles?.map(profile => ({
                id: `${item.id}-${profile.id}`,
                profileType: profile.profile,
                length: parseFloat(profile.measurement),
                quantity: profile.quantity,
                totalLength: parseFloat(profile.measurement) * profile.quantity,
                workOrderId: item.workOrderId,
                color: item.color,
    ...
    if (items.length === 0) {
      setOptimizationResult({
        success: false,
        message: 'Seçilen kesim listesinde hiç iş emri bulunmuyor. Lütfen Kesim Listesi Oluşturucu sayfasından iş emri ekleyin.',
        error: 'NO_ITEMS'
      });
      return;
    }
    ...
    try {
      ...
      const workOrderIds = Array.from(new Set(
        items.map(item => item.workOrderId).filter(Boolean)
      ));
      const profileTypes = Array.from(new Set(
        items.map(item => item.profileType).filter(Boolean)
      ));
      ...
      console.log('[EnterpriseWizard] REQUEST BODY:', {
        ...
        stockLengths: requestBody.materialStockLengths.map(s => s.stockLength),
        workOrderIds: requestBody.items.map(i => i.workOrderId).filter(Boolean),
        profileParams: (requestBody as { profileParams?: unknown }).profileParams,
      ...
      console.log('[EnterpriseWizard] 🔍 OptimizationResult:', result.data?.optimizationResult);
      console.log('[EnterpriseWizard] 🔍 OptimizationResult cuts:', result.data?.optimizationResult?.cuts);
      ...
      console.log('[EnterpriseWizard] 🎯 Received cuts detail:', {
        ...
        firstSegmentWorkOrder: result.data.cuts?.[0]?.segments?.[0]?.workOrderId,
        allStockLengths: Array.from(new Set(result.data.cuts?.map((c: Cut) => c.stockLength))),
        uniqueWorkOrders: Array.from(new Set(
      ...
      if (result.success && result.data) {
        ...
        if (result.data.paretoFront) {
          console.log('[EnterpriseWizard] Pareto optimization completed:', result.data.frontSize, 'solutions');
          ...
          console.log('[EnterpriseWizard] 🎯 Setting algorithm from result.data.algorithm:', result.data.algorithm);
          console.log('[EnterpriseWizard] 🎯 Type:', typeof result.data.algorithm);
          console.log('[EnterpriseWizard] 🎯 Full data object keys:', Object.keys(result.data));
          console.log('[EnterpriseWizard] 🎯 FULL DATA:', JSON.stringify(result.data, null, 2).substring(0, 500));
          
          console.log('[EnterpriseWizard] 🔍 Setting optimization result:', {
            hasMetadata: !!result.data.metadata,
            metadata: result.data.metadata,
            metadataKeys: result.data.metadata ? Object.keys(result.data.metadata) : []
          });
  ...
  const canStartOptimization = useMemo(() => {
    return !!(selectedCuttingList && params.objectives?.length > 0 && !isOptimizing);
    ...
    >
        ...
        >
          ...
          >
            {STEPS.map((step, idx) => {
              ...
              const isClickable =
                isCompleted || isActive || (step.id === activeStep + 1 && completedSteps.includes(activeStep));

              return (
                <React.Fragment key={step.id}>
                  <Chip
                    label={step.label}
                    onClick={isClickable ? () => handleStepChange(step.id) : undefined}
                  ...
                  />
                  {idx < STEPS.length - 1 && (
                    <ChevronRightIcon fontSize="small" sx={{ color: ds.colors.text.secondary }} />
                  )}
                </React.Fragment>
              );
            })}
          ...
          >
            <Stack spacing={ds.spacing['3']}>
              <ParametersStep
                ...
                itemCount={selectedCuttingList?.sections?.reduce((sum, s) => sum + s.items.length, 0) ?? 0}
              />

              <Stack direction="row" spacing={ds.spacing['1']}>
                <Chip
                  icon={<CheckCircleIcon />}
                  label={selectedCuttingList ? 'Liste seçildi' : 'Liste bekleniyor'}
      ...
      {selectedCuttingList && (
        <DetailedSelectionDialog
          ...
          onClose={() => {
            console.log('[EnterpriseWizard v2] Dialog closed without selection');
          ...
          onConfirm={(items, selectedProfiles) => {
            ...
            console.log('[EnterpriseWizard v2] 🔍 Checkpoint 2.1 - Before Transformation:', {
            ...
            const optimizationItems = items.map(item => {
              ...
              if (!item.profile || item.profile.trim() === '') {
                console.warn('[EnterpriseWizard v2] ⚠️ Empty profile type for item:', item);
              }
              if (isNaN(parsedLength) || parsedLength <= 0) {
                console.warn('[EnterpriseWizard v2] ⚠️ Invalid measurement for item:', item, 'parsed as:', parsedLength);
              }
              if (!item.quantity || item.quantity <= 0) {
                console.warn('[EnterpriseWizard v2] ⚠️ Invalid quantity for item:', item);
              }
            ...
            console.log('[EnterpriseWizard v2] 🔍 Checkpoint 2.1 - After Transformation:', {
              optimizationItemsCount: optimizationItems.length,
              sampleTransformedItems: optimizationItems.slice(0, 2),
              totalLengthSum: optimizationItems.reduce((sum, item) => sum + (item.length * item.quantity), 0),
              uniqueWorkOrders: new Set(optimizationItems.map(item => item.workOrderId)).size,
              timestamp: Date.now(),
            });
            ...
            Object.entries(selectedProfiles).forEach(([profileType, profile]) => {
            ...
            console.log('[EnterpriseWizard v2] 📦 Stock lengths from profiles:', {
            ...
            setParams(prev => {
              ...
              console.log('[EnterpriseWizard v2] 🔍 Checkpoint 2.2 - Params State Updated:', {
      ...
      <StockLengthConfigDialog
        ...
        onClose={() => {
          console.log('[EnterpriseWizard v2] Stock config dialog closed without confirmation');
          setShowStockLengthConfig(false);
        }}
        initialStockLengths={params.stockLengths}
        onConfirm={(stockLengths) => {
          console.log('[EnterpriseWizard v2] Stock lengths configured:', stockLengths);

...
import { LengthUnit, PreviewMetrics, OptimizationRequest, OptimizationItem } from '../types';
...
export const buildOptimizationRequest = (
  ...
  return {
    ...
    objectives: params.objectives.map(obj => ({
      type: obj.type as 'minimize-waste' | 'maximize-efficiency' | 'minimize-cost' | 'minimize-time' | 'maximize-quality',
      weight: obj.weight,
...
export const validateCuttingList = (cuttingList: Array<{ length: number; quantity: number }>): boolean => {
  return cuttingList.length > 0 && 
...
export const validateOptimizationParams = (params: {
  ...
  const objectivesValid = params.objectives.length > 0 && 
                         params.objectives.every(obj => obj.weight >= 0);
  const constraintsValid = params.constraints.kerfWidth >= 0 && 
                          params.constraints.maxWastePercentage >= 0 && 
                          params.constraints.maxWastePercentage <= 100;
  const stockValid = params.stockLengths.length > 0 && 
...
export const normalizeWeights = (objectives: Array<{ weight: number }>): Array<{ weight: number }> => {

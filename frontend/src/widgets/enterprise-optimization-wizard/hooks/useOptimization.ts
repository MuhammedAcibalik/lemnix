...
import { buildOptimizationRequest, transformToOptimizationItems, debugLog, handleApiError } from '../utils';
import { API_ENDPOINTS } from '../constants';

export const useOptimization = () => {
  // Run single algorithm optimization
  const runOptimization = useCallback(async (state: WizardState): Promise<boolean> => {
    const { cuttingList, params } = state;
    
    try {
      // Transform cutting list to optimization items
      const items = transformToOptimizationItems(cuttingList as Array<{
        profileType: string;
        length: number;
        quantity: number;
        workOrderId: string;
      ...
      debugLog('Frontend: First 3 items', items.slice(0, 3).map(i => ({ length: i.length, quantity: i.quantity })));
      ...
      if (result.success) {
        ...
        debugLog('result.data.optimizationResult', result.data.optimizationResult);
        return result.data.optimizationResult;
      } else {
        throw new Error(result.error || 'Optimization failed');
      }
    } catch (error) {
      const errorMessage = handleApiError(error, 'Error running optimization');
  ...
  const compareAlgorithms = useCallback(async (state: WizardState): Promise<boolean> => {
    const { cuttingList, selectedAlgorithms } = state;
    
    try {
      const items = transformToOptimizationItems(cuttingList.map(item => ({
        ...item,
      ...
      const errorMessage = handleApiError(error, 'Error comparing algorithms');

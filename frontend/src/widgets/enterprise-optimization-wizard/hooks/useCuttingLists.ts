...
export const useCuttingLists = () => {
  ...
  const addSampleItems = useCallback((cuttingList: CuttingListItem[], setCuttingList: (items: CuttingListItem[]) => void) => {
  ...
  const handleDetailedSelection = useCallback((list: Record<string, unknown>, callbacks: {
  ...
  const handleSelectionFromDetailed = useCallback((items: CuttingListItem[], setCuttingList: (items: CuttingListItem[]) => void) => {
    ...
    setCuttingList(convertedItems as Array<{
      id: string;
      workOrderId: string;
      color: string;
      version: string;
      ...
      profiles: Array<{ id: string; profile: string; measurement: string; quantity: number }>;
    }>);
  }, []);

  // Handle conversion complete from detailed selector
  const handleConversionFromDetailed = useCallback((result: ConversionResult, callbacks: {
